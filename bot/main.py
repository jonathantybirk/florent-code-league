...
Design rules, every one derived from a measured engine fact (llm-slop-analysis/elias/ground-truth.md):
...
SIEGE_BUILDERS = 10
# Consecutive rounds a Builder Bot may sit on our chosen firing tile before we re-derive around
# it. A body is not a building, so the occupancy check upstream misses it entirely and the build
# branch spins forever (measured: gunners built in 1 of 42 games against a bot whose whole
# strategy is parking six builders and never acting again). Deliberately generous -- a rival
# WALKING across the tile clears in one or two rounds and must not cost us a replan; only a
...
VAULT_GAIN = 4
# Rounds a spent ferry Launcher waits, after the rush has completed and with nothing in its pickup
# ring, before retiring itself to refund its +10 percentage points of global cost scale. Generous
# on purpose: an enemy rusher walking the same corridor is worth far more thrown than the scale is
# worth refunded, so this only reclaims Launchers on ground nobody is using.
RETIRE_IDLE = 25
# THE GUARD LAUNCHER -- displacement used defensively rather than as a ferry.
#
# Siting is the whole game here, and it was measured the wrong way round first. A Launcher on our
# own Core ring -- the obvious reading of "park one on our approach" -- made ZERO throws in 43
# rounds against `tempest_fast`, because a Gunner only needs r^2 <= 13 to bear on a Core footprint
# tile, so rivals stop 3-5 tiles short and never enter a ring-sited Launcher's pickup radius at
# all. Re-sited 3-4 tiles out it began intercepting immediately, and a scout across four rivals on
# five maps found the best single tile sees 9-48 enemy-builder-rounds inside its ring, consistently
# at Chebyshev 3-5 from our footprint toward the map centre.
#
# One throw is 0 titanium, 0 ammunition and a measured +4 tiles of displacement -- four rounds off
# their siege, every round, forever. What it is NOT is area defence: pickup is r^2 <= 2, so one
# Launcher covers 8 tiles and nothing else. Against a pathfinder that routes around the ring it
# managed 1 throw in 986 rounds. It guards a tile, not a base.
GUARD_MIN_CHEB = 2
GUARD_MAX_CHEB = 4
# Titanium kept back so a guard never starves the siege that actually wins games.
GUARD_FLOOR = 90
# Never before the opening is over.
GUARD_FROM = 12
...
class Player:
    def __init__(self):
        ...
        self.rush_body = 0             # consecutive rounds a BODY has held the firing tile
        self.rush_body_xy = None       # which tile that streak belongs to
        self.idle_since = 0            # rounds a spent Launcher has had nothing to throw
        self.guard_built = False       # this builder has planted its one guard Launcher
        ...
        if pos.distance_squared(bpos) == 1:
            ...
            try:
                if kind == "gunner":
                    ...
                    # `siege.plan` only ever proposes this when NO Gunner lane exists at all --
                    # a walled approach, or a defender who has bricked its own Core ring. The
                    # Sentinel shoots through both. Facing is permanent: `can_rotate` is False
                    # on a Sentinel and `rotate` raises `Unit is not a gunner`.
                    d = DIR_BY_NAME.get(facing)
                    if d is not None and ct.can_build_sentinel(bpos, d):
                        ct.build_sentinel(bpos, d)
                        ok = True
                elif kind == "conveyor":
                    d = DIR_BY_NAME.get(facing)
            ...
            #
            # WHAT IS GIVEN UP ON, since 2026-08-02: the tile being physically held by a BODY.
            # That is the case the timer could not tell apart from "not funded yet", and it is
            # the difference between waiting productively and not waiting at all. A Builder Bot
            # is not a building, so `_building_at` above returns None and we fall through here
            # with `can_build_gunner` permanently False. Measured against `a4sit` -- sixty lines
            # that park six builders on ring tiles and never act again -- the rusher built
            # gunners in 1 of 42 games, failing 1,136 times per game, because nothing in this
            # branch ever notices a body. Detecting it directly needs no timer: if a body holds
            # the tile for BODY_PATIENCE consecutive rounds it is a squat, not a queue, and the
            # tile is worth re-deriving around. An unfunded tile has no body and never trips it.
            if not ok:
                body = self._bot_at(ct, bpos)
                if body is not None:
                    if self.rush_body_xy != bxy:
                        self.rush_body_xy = bxy
                        self.rush_body = 0
                    self.rush_body += 1
                    if self.rush_body >= BODY_PATIENCE:
                        self.rush_body = 0
                        self.rush_body_xy = None
                        if self.rush_i == 0:
                            self.rush_black.add(bxy)
                            self._drop_plan()
                        else:
    ...
    def _is_our_gunner(self, ct, entity_id):
        """Our own siege turret standing on a planned firing tile -- Gunner or Sentinel.

        Sentinels are included because `siege.plan` now proposes one when no Gunner lane exists at
        all; without this the rusher would treat the turret it just paid 30 Ti for as somebody
        else's building, blacklist the tile and re-derive the whole plan around it.
        """
        if entity_id is None:
            return False
        try:
            return (ct.get_entity_type(entity_id) in (EntityType.GUNNER, EntityType.SENTINEL)
    ...
    def _run_launcher(self, ct):
        ...
        if self._displace(ct, pos, w, h, rusher):
            self.idle_since = 0
            return
        # RETIRE A SPENT LAUNCHER. The global cost scale is a LIVE census, not a cumulative
        # ratchet: `scale% = 100 + sum of the increment of every LIVING owned entity`, and removal
        # refunds in full and immediately (1224 measured checks across 153 scale values, zero
        # mismatches; self_destruct, destroy and death by enemy fire are identical). A Launcher is
        # +10 percentage points on the price of every gunner, barrier and harvester we buy for the
        # rest of the match, and the dead-code audit measured 3.2-3.8 Launchers BUILT per game for
        # 1.6-1.9 actual throws -- roughly half of them are pure tax on a relay that already ran.
        # Retired only once the rush is complete AND nothing has come within pickup range for a
        # while, so the displacement value above is kept rather than traded away.
        if done:
            self.idle_since += 1
            # A Launcher standing over our own Core is a BODYGUARD, not a spent relay, and is never
            # retired. Measured: four Launchers on the footprint diagonals held a Core at 500/500
            # for 995 rounds against four building-shooting builders, where the identical bot
            # without them lost its Core on turn 148. Only corridor Launchers -- the ones the ferry
            # left behind out in the map, which the audit measured at 3.2-3.8 built per game for
            # 1.6-1.9 throws -- are worth trading back for their +10 percentage points of scale.
            if (self.idle_since >= RETIRE_IDLE
                    and not self._near_home(ct, pos)
                    and not self._foe_in_ring(ct, pos, w, h)):
                try:
                    ct.self_destruct()
                except Exception:
                    self.idle_since = 0
    ...
    def _near_home(self, ct, pos):
        """Is this unit close enough to our own Core to be doing guard duty there?"""
        try:
            cx, cy = ct.read_store(S_CORE_X), ct.read_store(S_CORE_Y)
        except Exception:
            return False
        return max(abs(pos.x - cx), abs(pos.y - cy)) <= GUARD_MAX_CHEB + 1

    def _foe_in_ring(self, ct, pos, w, h):
        """Any ENEMY Builder Bot inside this Launcher's pickup ring (r^2 <= 2, the 8-neighbourhood).

        Measured directly: `can_launch` is True at d^2 = 1 and 2 and False at 4, 9, 16 and 25, so
        the ring is exactly the Chebyshev-1 neighbourhood and nothing beyond it.
        """
        for dx in (-1, 0, 1):
            for dy in (-1, 0, 1):
                if dx == 0 and dy == 0:
                ...
                if not (0 <= t.x < w and 0 <= t.y < h):
                    continue
                if self._is_enemy(ct, self._bot_at(ct, t)):
                    return True
        return False
        ...
        try:
            ...
            if found is None:
                ...
                found = siege.plan(w, h, anchor, self.core_tiles, walls, self._own_built(),
                                   ...
                                   blacklist=self.rush_black, known=known)
            if found is None:
                # THIRD PASS: a SENTINEL. Both Gunner passes have come back empty, which means
                # there is no tile on this map with a clear bead onto the enemy Core -- a walled
                # approach, or a defender that has bricked its own 12-tile ring. That is precisely
                # the state in which this bot used to bank titanium and collect nothing: measured
                # against a ring-bricking fort it managed 2 core kills in 42 games while ending
                # several with thousands of titanium unspent.
                #
                # A Sentinel has a bead where a Gunner has none, because its shot is stopped by
                # nothing at all. Verified in-engine on `maps/lab/sentwall.map26`: one behind a
                # two-tile wall band killed a 500 HP Core at turn 84, the exact arithmetic of
                # 28 shots at 18 damage on a 3-round cooldown.
                #
                # LAST, never first. It is 30 Ti and +20 percentage points against 10 and +10, and
                # 2.78x worse per titanium of damage -- so it must not pre-empt a permissive Gunner
                # that the executor could still take.
                found = siege.plan(w, h, anchor, self.core_tiles, walls, self._own_built(),
                                   self._flood(ct, (pos.x, pos.y), blocked, w, h),
                                   blacklist=self.rush_black, known=known, sentinel=True)
        except Exception:
            found = None
        if found is None:
            if self.rush_i == 0:
    ...
    def _hold_home(self, ct, pos):
        ...
        if not (self.alarm or (self._pv("hold_on_posture")
            ...
            return False
        # HEAL BEFORE BRICK, and this ordering has now been challenged and kept.
        #
        # The audit makes the case against it look overwhelming: 1,127 alarm-up rounds produced 944
        # heals and 8 barriers in 210 games -- 0.038 a game -- because during an alarm the Core is
        # damaged by definition, so a builder standing on it always has a damaged neighbour and
        # returns here before the fortify branch is ever reached. The lab arithmetic agrees:
        # a Gunner's ray stops at the FIRST building, and 201 rounds of sustained fire against one
        # builder rebuilding one barrier cost 1.01 Ti a round with the Core taking ZERO damage,
        # against healing's 4 HP per titanium versus 10 damage a round arriving.
        #
        # It MEASURED WORSE anyway: promoting fortify ahead of heal took `undertow` from 31-11 to
        # 29-13 over 42 mirrored games, with core kills falling 31 -> 29, while `vanguard` and
        # `tempest_fast` were unmoved. The lab result does not transfer because the lab put the
        # barrier exactly in the lane. Here there are 12 ring tiles, the threat-side sort is a
        # guess at which one the enemy Gunner's ray actually uses, and a builder that walks to
        # brick the wrong tile has stopped healing for nothing.
        #
        # The transferable version is not "brick sooner", it is "brick the RIGHT tile": enemy
        # turret facings are readable (`get_direction` works on enemy turrets), so the tile at
        # `enemy_gunner_pos + facing.delta()` is knowable exactly rather than guessed. That is a
        # real change with a real measurement behind it (a 3 Ti brick there burned enemy ammunition
        # at 2:1 and blocked the lane 82.4% of rounds) and it is NOT what this branch does. Left
        # as it is until that is built and measured properly.
        if self._heal(ct, pos):
            return True
        if self._guard(ct, pos):
            return True
        try:
    ...
    def _foe_near_home(self, ct, pos):
        """An enemy Builder Bot in vision, close enough to our Core to be worth throwing."""
        if self.core_pos is None:
            return False
        cx, cy = self.core_pos
        try:
            ...
            return False
        for bid in ids:
            # `get_nearby_units` RETURNS BUILDINGS -- Cores and turrets are in this list despite the
            # stub saying "ids of all units". Anything sizing an enemy force from it unfiltered is
            # counting their Core and their turrets as crew.
            try:
                ...
                continue
            if max(abs(p.x - cx), abs(p.y - cy)) <= GUARD_MAX_CHEB + 2:
                return True
        return False

    def _guard(self, ct, pos):
        """Plant ONE guard Launcher on the enemy's approach. True if the round was spent.

        The defensive half of the launcher arsenal. Throwing is free -- no titanium, no ammunition,
        cooldown 1 -- and lands a measured +4 tiles every time, so a guard that fires even
        occasionally is four rounds off their siege for a one-off 20 Ti. `_displace` above does the
        throwing; this only ever decides whether a Launcher exists to do it.

        Sited at Chebyshev GUARD_MIN_CHEB..GUARD_MAX_CHEB from our Core toward the enemy, never on
        the ring: a ring-sited Launcher measured ZERO throws in 43 rounds because a Gunner bears on
        our Core from 3-5 tiles out and its crew never comes closer than that.

        Exactly one, and only under alarm. A second is 20 Ti and +10 percentage points of scale on
        every later purchase for a second 8-tile patch, and the siege is what wins games.
        """
        if self.core_pos is None or self.enemy_anchor is None:
            return False
        if self.guard_built or not self.alarm:
            return False
        # Only when there is actually something to throw. A Launcher's pickup ring is r^2 <= 2 --
        # eight tiles -- so one built against an opponent whose crew never comes within Chebyshev 1
        # of it is 20 Ti and +10 percentage points of permanent scale for nothing.
        if not self._foe_near_home(ct, pos):
            return False
        try:
            if ct.get_current_round() < GUARD_FROM:
                return False
            if ct.get_global_resources() < ct.get_launcher_cost() + GUARD_FLOOR:
                return False
            w, h = ct.get_map_width(), ct.get_map_height()
        except Exception:
            return False
        ...
        best = None
        best_key = None
        for dx in range(-GUARD_MAX_CHEB, GUARD_MAX_CHEB + 1):
            for dy in range(-GUARD_MAX_CHEB, GUARD_MAX_CHEB + 1):
                cheb = max(abs(dx), abs(dy))
                if cheb < GUARD_MIN_CHEB or cheb > GUARD_MAX_CHEB:
                    continue
                t = Position(cx + dx, cy + dy)
                if not (0 <= t.x < w and 0 <= t.y < h):
                    continue
                if (t.x, t.y) in self.core_tiles or (t.x, t.y) in self.known_walls:
                    continue
                if self._building_at(ct, t) is not None:
                    continue
                # Toward the enemy: the tiles their rusher actually walks.
                key = ((t.x - ax) ** 2 + (t.y - ay) ** 2,
                       pos.distance_squared(t), t.x, t.y)
                if best_key is None or key < best_key:
                    best, best_key = t, key
        # OPPORTUNISTIC ONLY: build it if we happen to be standing next to the tile, never walk to
        # it. The walking version measured catastrophically -- 32-10 to 20-22 against `vanguard`,
        # 31-11 to 19-23 against `undertow` -- because returning True while in transit hijacks the
        # round from healing and from the economy, every round, for a Launcher that may never see
        # anything enter its 8-tile pickup ring.
        if best is None or pos.distance_squared(best) != 1:
            return False
        if not self._can_act(ct):
            return False
        try:
            if ct.can_build_launcher(best):
                ct.build_launcher(best)
                self.guard_built = True
                return True
        except Exception:
            pass
        return False
    ...
    def _run_sentinel(self, ct):
        ...
        try:
            ...
            return
        # THE CORE FIRST, always. A Sentinel's shot is not stopped by anything -- measured through a
        # two-tile wall band -- so unlike a Gunner it is never forced to chew what stands in front
        # of it. Taking targets in the order the engine happens to list them would spend 10 ammo a
        # shot on whatever barrier the defender dropped nearest us, which is precisely the trade
        # the Sentinel exists to refuse. A 500 HP Core is 28 shots; nothing else on the lane is
        # worth delaying that.
        best = None
        for tile in tiles:
            ...
            if not self._is_enemy(ct, occupant):
                continue
            core = (tile.x, tile.y) in self.enemy_core_tiles
            if not core:
                try:
                    core = ct.get_entity_type(occupant) == EntityType.CORE
                except Exception:
                    core = False
            if core:
                best = tile
