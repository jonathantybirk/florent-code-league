...
Design rules, every one derived from a measured engine fact (docs/ground-truth.md):
  ...
  G53      Never route ANY chain into a turret. A turret accepts nothing at all from a conveyor and
           the belt jams permanently; there is no ammo logistics branch to separate, because on
           2.3.3 ammunition is a team-wide pool filled only by convert_ammo() at the Core (G52).
  G59      A Builder Bot acts on ORTHOGONALLY ADJACENT tiles only -- never a diagonal, never its
           own tile. Build, attack, heal and destroy all share that radius.
...
# Rounds a rusher will stand on a legal build tile whose `can_build_*` keeps answering False
# before it gives that step up. Deliberately long: the usual cause is that the economy has not
# banked the Gunner's cost yet, and abandoning a firing position over a temporary shortfall is
# far more expensive than waiting. It exists only so a build that is PERMANENTLY illegal cannot
# park the rusher for a thousand rounds.
BUILD_PATIENCE = 45
...
def _gunners_only(route):
    """Strip the dead logistics out of a siege route (G53).

    `siege.plan` returns gunner -> conveyors -> harvester: a forward producer feeding the turret
    through a short belt, which is what fed a turret on 2.2.0. On 2.3.3 a turret accepts NOTHING
    from a conveyor -- the belt picks up one stack and holds that same stack id eighty rounds
    later, and the global ammo pool never moves -- so every one of those steps buys exactly zero
    damage. Worse than zero, in fact: a Harvester is 20 base titanium and +5 percentage points of
    PERMANENT global cost scale (G07), each conveyor another +1, and the only place ammunition
    can now come from is `convert_ammo` at our own Core, which is paid for out of the same bank.

    Left in siege.py rather than removed there, because the planner's ranking is owned elsewhere
    and this survives any shape it takes: if the conveyor/harvester steps stop being generated,
    this filter becomes a no-op.
    """
    return tuple(s for s in route if s[0] == "gunner")
...
class Player:
    def __init__(self):
        ...
        self.ring_tiles = None         # the full legal spawn ring, legacy order first (G64a)
        ...
        self.build_wait = 0            # rounds spent adjacent to a build we cannot yet make
    ...
    def _ring_tiles(self, ct, pos):
        """The full legal spawn ring, legacy order first (G63, G64a).

        This used to be exactly `tuple(pos.add(d) for d in ALL_DIRS)`, and `pos` is the TOP-LEFT
        anchor of the 2x2 footprint (G33): three of those eight offsets land ON the Core, so only
        five of the twelve legal ring tiles were ever candidates. On the 15-map pool that was a
        bias and nothing worse, because every Core there sat two tiles clear of a border.

        Five of the six NEW maps put a Core FLUSH AGAINST A MAP EDGE (G64a), and there the same
        eight offsets go off-map instead. Counted over all 21 maps x both sides: jackpot/a,
        sweden/a and vase/a leave **zero** legal ALL_DIRS tiles -- the Core there could not spawn
        a single builder past the two attackers, and the economy simply did not exist -- while
        bridge/a, string/a, string/b, sweden/b and vase/b leave two.

        The legacy eight are kept in front, so on the sixteen maps where the old list already
        worked the Core picks exactly the tile it picked before and nothing moves. The remaining
        ring tiles are appended, never substituted.
        """
        if self.ring_tiles is not None:
            return self.ring_tiles
        legacy = tuple(pos.add(d) for d in ALL_DIRS)
        try:
            w, h = ct.get_map_width(), ct.get_map_height()
        except Exception:
            return legacy
        foot = set(siege.footprint((pos.x, pos.y))) if siege is not None else set()
        foot |= self.core_tiles
        out, seen = [], set()
        for t in legacy:
            key = (t.x, t.y)
            if key in seen or key in foot or not (0 <= t.x < w and 0 <= t.y < h):
                continue
            seen.add(key)
            out.append(t)
        for dx in range(-1, 3):
            for dy in range(-1, 3):
                ...
                if key in seen or key in foot:
                    continue
                if not (0 <= key[0] < w and 0 <= key[1] < h):
                    continue
                seen.add(key)
                out.append(Position(key[0], key[1]))
        self.ring_tiles = tuple(out)
        return self.ring_tiles
    ...
    # `_builder_target` lived here -- an ore-count-scaled builder cap. It was dead code with a
    # live NameError in it: MIN_BUILDERS and MAX_BUILDERS do not exist anywhere in this module,
    # so any future caller would have taken a NameError straight into the blanket handler in
    # `run()`, and on the Core that is one silent lost turn per round. Removed.
    ...
    def _run_rush(self, ct, pos):
        ...
        # ORTHOGONALLY ADJACENT ONLY (G59). This gate was `<= 2` -- written when the action radius
        # measured r^2 <= 2 and diagonal builds landed (old G50). 2.3.3 reversed that: every
        # `can_build_*` is False on a diagonal and every `build_*` raises there.
        #
        # The failure was not a raise, it was a HANG. The moment the rusher stepped onto any tile
        # at d2 <= 2 of its build target this branch took the round; on a diagonal every
        # `can_build_*` answered False, `ok` stayed False, and the function returned True having
        # neither built, moved, nor touched `rush_stuck` -- so the 12-round give-up never fired
        # either. The rusher stood on that diagonal for the rest of the match. `_bfs_step` seeded
        # its goal ring with all EIGHT neighbours, so it steered into exactly that tile by design.
        if pos.distance_squared(bpos) == 1:
            ...
            if ok:
                ...
                # `can_build_*` said no from a legal tile. Nearly always "not enough titanium
                ...
                # RUSH_RESERVE for exactly this purchase. But it can also be permanent (the unit
                # cap, terrain the planner mis-remembered), and a rusher waiting on a build that
                # will never be legal has deleted itself. Generous, therefore, but bounded.
                self.build_wait += 1
                if self.build_wait >= BUILD_PATIENCE:
                    self.build_wait = 0
                    if self.rush_i >= self.rush_base_len > 0:
                        self._abort_segment(bxy)
                    else:
                        self.rush_black.add(bxy)
                        self._drop_plan()
    ...
    def _abort_segment(self, bxy):
        ...
        if self.rush_route and self.rush_base_len and self.rush_seg >= self.rush_base_len > 0:
            ...
            self.feeders = tuple(s[1] for s in self.rush_route if s[0] == "gunner")
    ...
    def _extend_deposit(self, ct, pos):
        ...
        # 2 games out of 30.
        try:
            if ct.get_global_resources() < ct.get_gunner_cost() + DEPOSIT_RESERVE:
                return False
        except Exception:
            return False
        add = _gunners_only(found["route"])
        if not add:
            return False
        self.rush_seg = len(self.rush_route)
        self.rush_route = tuple(self.rush_route) + add
        self.rush_ray = frozenset(self.rush_ray) | found["ray"]
        self.feeders = tuple(self.feeders) + tuple(s[1] for s in add)
        self.deposit_n += 1
    ...
    def _plan_rush(self, ct, pos):
        ...
        if found is None:
            if self.rush_i == 0:
                ...
                self.rush_target = None
            return
        route = _gunners_only(found["route"])
        if not route:
            return
        if self.rush_route is not None and route == self.rush_route:
            return
        self.rush_route = route
        self.rush_target = anchor
        self.rush_ray = found["ray"]
        self.rush_i = 0
        self.rush_dist = None
        self.rush_feeder = found["gunner"]
        self.rush_base_len = len(route)
        self.battery_n = 0
        self.battery_round = -999
        # `feeders` used to be the ore tile the route's Harvester stood on -- the thing that
        # supplied the turret. On 2.3.3 nothing supplies a turret except the Core (G52/G53), so
        # the anchor `battery()` packs extra angles around is the TURRET WE ALREADY BUILT. Its
        # other two uses fall out correctly: `_live_feeders` checks that a building still stands
        # on the tile, and a Gunner is a building; and `battery()` refuses to lay a lane across a
        # feeder, which now means "never point a new Gunner at one of our own".
        self.feeders = tuple(s[1] for s in route)
        self.deposit_n = 1
        self.deposit_dry = 0
        self.deposit_round = -999
        self.rush_seg = 0
    ...
    def _bfs_step(self, ct, pos, goal):
        ...
        The ring is the four cardinals because the build radius is the four cardinals (G59). It
        was seeded with all eight, on the old G50 belief that a builder could act at r^2 <= 2 --
        which on 2.3.3 walks the rusher onto a diagonal, where the executor can never build and
        (before the fix above) never moved again either. Seeding the four is not merely the
        matching change, it is the load-bearing half of it: leave this at eight and the walk
        still terminates one tile short of anywhere the rusher can work from.
        """
        try:
            w, h = ct.get_map_width(), ct.get_map_height()
        except Exception:
    ...
    # `_sabotage` lived here: stand on an enemy conveyor and fire at your OWN tile, which on
    # 2.2.0 was the only attack a Builder Bot had (old G14). 2.3.3 deleted the own-tile shot --
    # `can_fire(own_position)` is False and `fire(own_position)` raises -- so the method was a
    # guaranteed no-op that cost one `can_fire` call per builder per round and, more to the
    # point, occupied the slot in `_run_builder` where a real builder attack belongs (G59: an
    # orthogonally adjacent tile, 2 damage for 2 Ti). Removed rather than repaired here; the
    ...
    def _run_sentinel(self, ct):
        """No get_sentinel_target() exists -- it raises on a Sentinel -- and a shot costs 10 of the
        team's global ammo against a Gunner's 2 (G55). Only ever fire at a confirmed enemy
        occupant. The pattern is a single-tile line of 5 at r^2 <= 32, not the 3-row band the
        2.2.0 register claimed (G15, refuted); nothing here depends on which."""
        try:
