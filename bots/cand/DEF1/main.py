...
# is corroboration and CANNOT reach the threshold however much of it accumulates: every remaining
# bit at once is 2+1+1+1+1+1 = 7 against a threshold of 8. That matters because the cheap bits are
# not cheap in the way they look: EV_INTRUDER and EV_DEEP fire in 90-100% of games against every
# opponent that spawns a Builder Bot at all, so a threshold they can reach between them is not a
# detector, it is "DEFENCE unless the enemy is inert".
EV_WEIGHTS = ((EV_TURRET_HURT, 8), (EV_MELEE, 2), (EV_HURT, 1),
