...
class Player:
    ...
    def _arbitrate(self, ct, rnd):
        ...
        for bit in EV_BITS:
            if (self.evidence & bit) and bit not in self.ev_round:
                ...
                _dl("EV|%d|%d|core=%d" % (rnd, bit, 1 if (self.core_ev & bit) else 0))
        ...
        self.switches += 1
        _dl("POST|%d|%d|score=%d|ev=%d" % (rnd, want, score, self.evidence))
        return self.posture
    ...
    def _run_core(self, ct):
        ...
        if rnd in (60, 100, 200, 400, 999):
            _dl("HB|%d|p=%d|ev=%d|score=%d|alarm=%d|sp=%d" % (
                rnd, self.posture, self.evidence, self.score,
                1 if self.alarm else 0, self.spawned))
