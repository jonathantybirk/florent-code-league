"""Core opening, doctrine choice, and persistent reinforcement spawning."""
...
from constants import (MAX_OPENING_BUILDERS, SLOT_BUILDER_HEARTBEAT,
...
def run(player: "Player", ct: Controller) -> None:
    ...
    if not hasattr(player, "builders_spawned"):
        ...
        # Round 0 is the only turn the doctrine gets to look at: the Core never
        # moves, so its vision never grows, and the Builders it is about to
        # spawn need their roles now. Every unit reads the answer back out of
        # SLOT_OWN_CORE rather than re-deriving it from its own vision, which
        # would not agree.
