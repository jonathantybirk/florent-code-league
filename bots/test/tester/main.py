...
class Player:
    def __init__(self):
        ...
        self.wall_pos: Position | None = None     # position of a wall we're trying to destroy
        # Cached core position (read from the store once, then reused)
        self.core_pos: Position | None = None
        self.adjecant: bool = False
    ...
    def _run_builder(self, ct: Controller) -> None:
        """Builder bot logic: move randomly until a wall is found, then fire at it until destroyed."""
        pos = ct.get_position()
        ...
        if self.wall_pos is None:
            self._random_move(ct)
            return
        
        dx = int(self.wall_pos.x - pos.x)
        dy = int(self.wall_pos.y - pos.y)
        manhattan = abs(dx) + abs(dy)

        if manhattan == 1:
            if ct.can_fire(self.wall_pos):
                ct.fire(self.wall_pos)
            # else:
            #     ct.self_destruct()
            return

        # Choose a cardinal step toward the wall
        if abs(dx) >= abs(dy):
            desired = Direction.EAST if dx > 0 else Direction.WEST
        else:
            desired = Direction.SOUTH if dy > 0 else Direction.NORTH

        if ct.can_move(desired):
            ct.move(desired)
            return

        # Can't move this round
    ...
    def _random_move(self, ct: Controller) -> None:
