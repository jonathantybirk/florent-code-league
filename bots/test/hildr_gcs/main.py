...
# hildr@01ee9ce rebuilt on the GCS: same play, but every store read/write goes
# through teamstate.TeamState (liveness, registry, map, STATUS) and all 16
# engine slots belong to the GCS protocol.  See teamstate.py for the mapping.
_GCS_KIND = {EntityType.CORE: "core", EntityType.BUILDER_BOT: "builder_bot",
             EntityType.GUNNER: "gunner", EntityType.SENTINEL: "sentinel",
             EntityType.LAUNCHER: "launcher"}
_FACING_INDEX = {Direction.NORTH: 0, Direction.NORTHEAST: 1, Direction.EAST: 2,
                 Direction.SOUTHEAST: 3, Direction.SOUTH: 4, Direction.SOUTHWEST: 5,
                 Direction.WEST: 6, Direction.NORTHWEST: 7}
_SYM_KIND = {'H': 0, 'V': 1, 'R': 2}        # hildr's letters -> protocol.SYMMETRY_KINDS
P.ONBOARD_ROUNDS = 0    # hildr spawns menders on consecutive rounds; a window every
                        # spawn would throttle that to one per ten rounds
...
class Player:
    ...
    def run(self, ct: Controller) -> None:
        ...
        try:
            ...
            import traceback
            print("GCSERR run " + traceback.format_exc().replace("\n", " | "))

    def _gcs_run(self, ct):
        kind = _GCS_KIND.get(ct.get_entity_type())
        if kind is None:
            return
        if self.gcs is None:
            self.gcs = GCS(kind, InternalMap(ct.get_map_width(), ct.get_map_height()))
            self.state = TeamState(self, {
                "BUILT": SLOT_BUILT, "BUILDER": SLOT_BUILDER, "ENEMY": SLOT_ENEMY,
                "THREAT": SLOT_THREAT, "BEAT0": SLOT_BEAT0, "BEATS": SLOT_BEATS,
                "HARVEST": SLOT_HARVEST, "EHEAL": SLOT_EHEAL, "GO": SLOT_GO,
                "EHP": SLOT_EHP, "MINER": SLOT_MINER, "ECON_OK": SLOT_ECON_OK,
                "ETA": SLOT_ETA})
        self.round = ct.get_current_round()
        result = self.gcs.absorb(ct)
        written_before = self.gcs.last_written
        self._gcs_symmetry(ct)
        try:
            self._run(ct)
        except Exception:
            pass                                   # as the original: never crash the unit
        try:
    ...
    def _gcs_symmetry(self, ct):
        """hildr knows the symmetry of known maps from its own Core's position;
        hand that to the map so enemy_core() works for every unit."""
        if self.gcs.map.symmetry() is not None:
            return
        own = self.gcs.map.our_core
        if own is None:
        ...
        if own is None:
            return
        try:
            kind, _ = _enemy_core(ct, Position(own[0], own[1]))
            self.gcs.map.set_symmetry(_SYM_KIND[kind])
        except Exception:
            pass

    def _gcs_spawn(self, ct, tile):
        """Spawn through the GCS: only outside resync/onboarding windows, and
        only when a slot can be handed out in the spawn round."""
        reg = self.gcs.registry
        if reg.in_onboard_window(self.round) or reg.is_resync_round(self.round):
            return False
    ...
    def _read(self, ct, slot, default=0):
        try:
            return self.state.read(ct, slot)
        except Exception:
            return default

    def _write(self, ct, slot, value):
        try:
            self.state.write(ct, slot, value)
        except Exception:
            pass
    ...
    def _spawn_one(self, ct, toward_enemy):
        ...
        try:
            ...
            if chosen is None:
                return False
            if not self._gcs_spawn(ct, chosen):
                return False
            self.spawned += 1
            return True
        except Exception:
    ...
    def _place(self, ct, here):
        ...
        for _d, dx, dy in CARDINALS:
            ...
            try:
                ...
                continue
            self.gcs.grant_turret(spot.x, spot.y, "sentinel", _FACING_INDEX[facing])
            self.built += 1
            self.blocked.add(key)
            self.occupied.add(key)
            self.path = []
    ...
    def _sentinel(self, ct):
        if self.enemy is None:
            ...
            self.enemy_tiles = _footprint(self.enemy)
        # heartbeat: the GCS liveness rule, nothing to write
        self._act(ct)
