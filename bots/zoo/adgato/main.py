"""FAITHFUL not-adgato CLONE -- rush four Sentinels, then FIVE Builders that only mend.

Decoded from five ladder replays of the rank-1 team: 5 builders, 4 sentinels, zero
harvesters or conveyors, and 510 heals across a 321-round game. Built from our own bot
with the economy switched off and the mender count raised, so the only difference from us
is what the post-rush Builders spend titanium on.

The previous sparring partner parked its Builders and never pressed, which flattered us
badly: it scored our econ build 30/30 while the real thing still beat us 0-5.
"""

_ORIGINAL_DOC = """Sentinel rush: one Builder walks to the enemy Core and stands up four Sentinels firing into it.
...
ECON_ROUND = 20            # if the Core is not dead by here, the rush failed -- start an economy
ECON_BUILDERS = 5          # Builders devoted to mining and mending once that happens
ECON_RESERVE = 140         # working capital kept liquid for Builders, belts and mending
...
class Player:
    ...
    def _keep_econ(self, ct):
        ...
        if self.round < ECON_ROUND or self.econ >= ECON_BUILDERS:
            return
        try:
    ...
    def _builder(self, ct):
        ...
        if self.role is None:
            # Anything spawned after ECON_ROUND exists only because the rush did not land, so
            # its job is the long game. No negotiation needed: the round it first ran decides.
            self.role = 'econ' if self.round >= ECON_ROUND else 'attack'
