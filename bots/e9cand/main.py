...
# Counted over the tiles a chain can actually TERMINATE on. Of the twelve ring tiles only the
# eight FACE tiles are orthogonally adjacent to the footprint, and only those can cap a chain
# (G02); the four CORNERS can never score a stack, so bricking one costs the economy exactly
# nothing and is done first. The old cap counted all twelve, and the ring is normally half full of
# our own belts already -- measured, `free` was 6 tiles and the cap left one single tile eligible
# on 472 of every 597 rounds the fortifier ran.
FORTIFY_KEEP_OPEN = 5
# Titanium the fortification leaves in the bank. NOT CHAIN_RESERVE, which is what it used to be
# and which is why the ring is inert: a Barrier is 3 base titanium and CHAIN_RESERVE is 45, so the
# gate asked for ~52 Ti at exactly the moment the bank is empty. Measured over 24 games against
# `vanguard` on unseen maps, the money gate rejected on 86 rounds a game and the ring got 0.75
# Barriers; theirs gets 8.93, and that arithmetic is the matchup -- 88% of their shots reach our
# Core, 40% of ours reach theirs.
#
# A builder that owes a belt is already excluded above by `self.owed`, and so is anything in the
# "belt" phase, so what this reserve protects is the NEXT link somebody else is about to lay --
# one conveyor, not one whole chain. An incomplete chain still scores exactly zero (G02); this
# does not spend into one, it stops pretending a 3 Ti wall costs 48.
FORTIFY_RESERVE = 12
...
# Times one turret may have its FEED re-planned under it before the rusher gives up on it. Each
# attempt blacklists the tile that failed, so the search strictly shrinks and this is a CPU guard
# rather than a behaviour knob: on a map whose forward half holds one deposit the second attempt
...
class Player:
    def __init__(self):
        ...
        self.occupied = set()
        # Occupancy split three ways, because the three behave nothing alike in a Gunner's lane.
        # `occupied_foe` is the subset an enemy owns -- 30 HP of Barrier a Gunner clears in three
        # shots, so a TARGET rather than a wall; ours in the same lane jams the turret for good
        # (G11) and is a hard obstacle. `producers` is every deposit already carrying a Harvester,
        # of EITHER team: a Harvester round-robins its output into every orthogonally adjacent
        # building and the API is team-blind, so one of theirs feeds our turret for nothing.
        self.occupied_foe = set()
        self.producers = set()
        self.occ_ver = 0
        ...
        self.deposit_dry = 0
        self.rush_seg = 0
        self.refeeds = 0               # feeds re-planned under a turret already standing
    ...
    def _run_rush(self, ct, pos):
        ...
        if occupant is not None:
            if kind == "gunner" and not self._is_our_gunner(ct, occupant):
                ...
                # Keyed on the STEP KIND rather than on `rush_i == 0`, so the turret at the head
                # of an APPENDED segment gets the same check the base route's always had.
                self._abort_segment(bxy)
                return True
            if kind == "conveyor" and not self._belt_ok(ct, occupant, facing):
                # A belt tile taken by anything that does not carry titanium the way this chain
                # needs it -- an enemy building, or (much more often) one of OUR OWN economy belts
                # running the other way, toward our Core. Stepping past it was silent and fatal:
                # the ammunition chain has a hole in it, so the turret at its head never receives
                # one stack. Measured on random-...-000: the rusher skipped two occupied belt
                # tiles at (3,14) and (4,14), reached a deposit already taken, and both of its
                # Gunners sat on ammo 0 for 180 rounds with the enemy Core in their sights.
                if self._refeed(ct, pos, bxy):
                    return True
                self._abort_segment(bxy)
                return True
    ...
    def _foe_buildings(self):
        """Remembered ENEMY buildings, Cores excluded -- the set a Gunner can shoot open."""
        foot = set(siege.footprint(self.enemy_anchor)) if (
            siege is not None and self.enemy_anchor is not None) else set()
        return (self.occupied_foe - self.core_tiles) - self.enemy_core_tiles - foot

    def _belt_ok(self, ct, entity_id, facing):
        """Does the building already on this belt tile carry ammunition the way the chain needs?

        Only one of ours, of the right kind, pointing the planned way. A conveyor of our own
        economy on the same tile faces the other way -- it carries the stack back to our Core,
        which scores titanium but leaves the turret empty -- so it is a BREAK in the ammunition
        chain, not a free segment, and the old code counted it as already built.
        """
        if entity_id is None:
            return False
        try:
            if ct.get_team(entity_id) != ct.get_team():
                return False
            if ct.get_entity_type(entity_id) not in (EntityType.CONVEYOR, EntityType.SPLITTER):
                return False
            want = DIR_BY_NAME.get(facing)
            if want is None:
                return True
            return ct.get_direction(entity_id) == want
        except Exception:
            return False

    def _refeed(self, ct, pos, failed):
        """Find another deposit for a turret we have ALREADY PAID FOR. True if the route changed.

        The measured defect this exists for: of every Gunner of ours that never fires, one hundred
        percent never held one unit of ammunition, and not one of 515 was aimed at the wrong Core
        -- 0.85 stranded turrets a game on the published maps and 0.62 on unseen ones, each 22 Ti
        and +10 points of permanent global cost scale (G07) for zero damage.

        The old answer to a taken deposit was to abort the segment, which on the base route means
        dropping the plan -- and dropping the plan abandons the turret too, then builds another one
        somewhere else that strands the same way. The turret is the expensive half and its lane is
        already clear, so it is the half worth keeping: this replaces only the FEED, leaving the
        Gunner exactly where it stands.

        Bounded by REFEED_MAX so a deposit-poor corner of the map cannot turn into a re-planning
        loop; the failed tile is blacklisted each time, so the search strictly shrinks anyway.
        """
        if siege is None or self.enemy_anchor is None or not self.rush_route:
            return False
        if self.refeeds >= REFEED_MAX or not self._cpu_left(ct):
            return False
        seg = self.rush_seg if (self.rush_seg and self.rush_i >= self.rush_seg) else 0
        route = tuple(self.rush_route)
        if seg >= len(route) or route[seg][0] != "gunner":
            return False
        gtile = route[seg][1]
        if not self._is_our_gunner(ct, self._building_at(ct, Position(gtile[0], gtile[1]))):
            return False        # nothing paid for yet -- the ordinary re-plan is cheaper
        self.rush_black.add(failed)
        try:
            w, h = ct.get_map_width(), ct.get_map_height()
        except Exception:
            return False
        ...
        try:
            steps = siege.feed(w, h, gtile, self.rush_ray, self.core_tiles, walls, ore,
                               buildings, dist, blacklist=black, known=known,
                               lanes=self.rush_ray, enemy_anchor=self.enemy_anchor,
        ...
        if steps is None:
            return False
        self.refeeds += 1
        self.rush_route = route[:seg + 1] + tuple(steps)
        self.rush_i = seg + 1
        self.rush_dist = None
        if seg == 0:
            self.rush_base_len = len(self.rush_route)
        self.feeders = tuple(s[1] for s in self.rush_route if s[0] == "harvester")
        self.deposit_n = max(1, len(self.feeders))
        return True
    ...
    def _live_feeders(self, ct):
        ...
        """
        out = []
        for f in tuple(self.feeders) + tuple(sorted(self.producers & self._near_front())):
            if f in out:
                continue
    ...
    def _near_front(self):
        """Deposits close enough to the enemy Core to be worth packing a turret around.

        A Gunner has to be orthogonally adjacent to the producer AND within three tiles of the
        enemy footprint, so anything further out than that can never yield a battery site and only
        costs the search time.
        """
        if self.enemy_anchor is None:
            return set()
        ax, ay = self.enemy_anchor
        return set(t for t in self.producers
                   if abs(t[0] - ax) <= 5 and abs(t[1] - ay) <= 5)
    ...
    def _extend_deposit(self, ct, pos):
        ...
        known = (self.seen | self.pred_seen) or None
        black = set(self.rush_black) | set(self.feeders)
        foe = self._foe_buildings()
        prod = self.producers - self.core_tiles - self.enemy_core_tiles - foot - set(self.feeders)
        try:
            ...
            if found is None:
                ...
                own_only = buildings - foe
                found = siege.plan(w, h, self.enemy_anchor, self.core_tiles, walls, ore,
                                   own_only,
                                   self._flood(ct, (pos.x, pos.y), blocked | own_only, w, h),
    ...
    def _extend_battery(self, ct, pos):
        ...
        try:
            sites = siege.battery(w, h, self.enemy_anchor, feeders, self.core_tiles,
                                  ...
                                  known=known, blacklist=self.rush_black, foe=foe)
            if not sites:
                ...
                own_only = buildings - foe
                sites = siege.battery(w, h, self.enemy_anchor, feeders, self.core_tiles,
                                      walls, ore, own_only, self.rush_ray,
                                      self._flood(ct, (pos.x, pos.y), blocked | own_only, w, h),
    ...
    def _plan_rush(self, ct, pos):
        ...
        known = (self.seen | self.pred_seen) or None
        foe = self._foe_buildings()
        prod = self.producers - self.core_tiles - self.enemy_core_tiles - foot
        try:
            found = siege.plan(w, h, anchor, self.core_tiles, walls, ore, buildings, dist,
                               blacklist=self.rush_black, known=known, producers=prod, foe=foe)
            if found is None:
                ...
                # Second pass with THEIR buildings forgotten for siting -- ours are still
                # obstacles. Occupancy memory never expires while a tile is out of vision, so a
                # single enemy conveyor glimpsed near their Core forty rounds ago used to delete
                # every bead through it permanently. Forgetting OUR OWN buildings as well, which
                # is what this pass used to do, is a different and worse bug: a lane laid through
                # our own turret or our own belt is jammed from the day it is built (G11).
                own_only = buildings - foe
                found = siege.plan(w, h, anchor, self.core_tiles, walls, ore, own_only,
                                   self._flood(ct, (pos.x, pos.y), blocked | own_only, w, h),
                                   blacklist=self.rush_black, known=known, producers=prod)
        except Exception:
            found = None
        if found is None:
            if self.rush_i == 0:
    ...
    def _rush_walk(self, ct, pos, target):
        ...
        if step is None:
            ...
            if self.rush_stuck >= 12:
                self.rush_stuck = 0
                if (self.rush_route and self.rush_i < len(self.rush_route)
                        and self.rush_route[self.rush_i][0] != "gunner"
                        and self._refeed(ct, pos, self.rush_route[self.rush_i][1])):
                    # A deposit or a belt tile we cannot walk to. That is a reason to feed the
                    # turret from somewhere else, not a reason to abandon a turret whose lane is
                    # already clear.
                    return True
        ...
        for tile in tiles:
            ...
            if occ is None:
                if key in self.occupied:
                    ...
                    self.occ_ver += 1
                self.occupied_foe.discard(key)
                self.producers.discard(key)
                ...
                # One extra pair of lookups on tiles that actually hold a building -- a handful a
                # round, against a 10 ms budget.
                try:
                    if ct.get_team(occ) != ct.get_team():
                        self.occupied_foe.add(key)
                    else:
                        self.occupied_foe.discard(key)
                    if ct.get_entity_type(occ) == EntityType.HARVESTER:
