...
def _build_siege_sentinel(p, ct, enemy_core):
    ...
    if _build_failure(p, ct, spot, "siege sentinel", ct.get_sentinel_cost()):
        ...
        p.siege_saving = 0
        return False
    # Standing beside a legal seat we cannot yet afford. Holding the turn here
    # is *saving up*: falling through buys a 20 Ti Gunner with the titanium the
    # 30 Ti Sentinel was three rounds away from, and 2.3.4 priced the Gunner as
    # the worse unit on every axis but that 10 Ti. Bounded, because the version
    # of this that waited forever is what parked Builder id=6 for 33 rounds.
    p.siege_saving = getattr(p, "siege_saving", 0) + 1
    if p.siege_saving < SIEGE_SAVE_ROUNDS:
        return True
    p.siege_saving = 0
