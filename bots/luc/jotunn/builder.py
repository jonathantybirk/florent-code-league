...
# The attacker Builder is an economy-denial specialist and never rushes
# the enemy Core. Off restores odin's rush attacker exactly.
DENIAL_SPECIALIST = True
...
def _run(p, ct):
    ...
    if _tend_duel_turret(p, ct):
        return
    # jotunn's divergence from the odin line. odin's third Builder races the
    # enemy Core; here it never attacks the Core at all and spends the whole
    # match on their economy -- barriers on their ore, cuts in their belts.
    #
    # This is a different win condition, not a tuned version of the same one.
    # A Core kill needs a Builder to out-trade turrets it cannot afford to
    # fight (2 damage a shot against 40 HP behind a Gunner). Denial needs
    # only to arrive: a 3 Ti barrier on an ore tile removes that tile's income
    # for the rest of the match, and the round-1000 tiebreak is decided on
    # titanium delivered. Against a bot that banks on the tiebreak, taking
    # their income is the same as scoring, and it costs a tenth as much.
    # BLITZ is exempt. That doctrine fires when the two Cores are close enough
    # that the match is a race, and a race is the one case where the Core kill
    # lands before denial could ever pay: traced on showdown, odin's attacker
    # takes the Core on round 27, long before a starved economy means anything.
    if (p.is_attacker and DENIAL_SPECIALIST
            and p.doctrine != doctrine.BLITZ):
        p.phase = "harass"
        _harass(p, ct)
        return
    if p.is_attacker:
