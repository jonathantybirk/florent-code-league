...
LAUNCHER_BUILDERS = 1
# The ring Builder moves to index 3 to make room for a second miner at index 1.
# Role arithmetic with PAD_FIRST_ORDER off: indices below `economy_builders` are
# miners, `ring_slot = index - LAUNCHER_BUILDER_INDEX` picks the ring, and
# everything else that is not the ring is the attacker. At 2 this gave
# 0=miner, 1=attacker, 2=ring; at 3 with economy_builders=2 it gives
# 0=miner, 1=miner, 2=attacker, 3=ring -- a second miner *added*, not
# reallocated from the attacker or the mender.
...
# Deposits `_pick` will evaluate in one round. Each candidate costs a `_route`
# and a `_distance` -- two searches -- and the list is already sorted
# nearest-first, so the tail is deposits this Builder was never going to take.
# Unbounded it is what put a second miner over the 10 ms turn limit: gefjon
# measured 11,237 us on aurora against steward_relent's 3,657, with nine turns
# over. Bounded by work, not by a clock, so it stays deterministic.
PICK_CANDIDATE_LIMIT = 4
NETWORK_CAP_EARLY = 4
NETWORK_CAP_LATE = 8
...
# A long-game lift of this cap (300+ rounds, 6 -> 10) was built and then cut:
# measured over 21 maps in both seats it changed nothing at all, because the cap
# is not what binds. Long games spawn about 5 Builders against a ceiling of 6;
# what actually stops the seventh is `resources >= builder_cost +
# ECON_EXPAND_RESERVE`, and `builder_cost` is inflated by the cost scale of the
# turrets this build is trying to stop paying for. If the Gunner change frees
# that scale, the reserve -- not the cap -- is the thing to revisit.
...
# The same arithmetic for the Gunner, which never had it. Deliberately longer
# than the Sentinel's 12: a Sentinel that stops shooting the Core has three
# ranked fallbacks and loses nothing by being early, while a Gunner that stops
# starts a clock that ends in self-destruction, so it should be the more
# reluctant of the two. 0 disables the check and restores the old behaviour.
GUNNER_SIEGE_STALL_ROUNDS = 20
