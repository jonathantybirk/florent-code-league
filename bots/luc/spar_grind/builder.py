...
from constants import (ANCHOR_ORE_ON_NETWORK, SIEGE_SENTINEL_FIRST_TRY,
    BELT_SCORE_CANDIDATES, BELT_TILE_WEIGHT,
...
def _pick(p, ct):
    ...
    # Rank deposits by how far they are from the *network*, not from the miner.
    #
    # The list is truncated to BELT_SCORE_CANDIDATES, so the ordering decides
    # what is even considered. Ordering by distance from the Builder means a
    # deposit sitting one tile off our existing belt is never looked at if the
    # Builder happens to be standing somewhere else, while a deposit close to
    # the Builder but fifteen tiles of belt from the Core is priced every time.
    #
    # Belt tiles are what a deposit actually costs: profiled over 300 rounds a
    # miner spends 299 of them laying and re-laying line and reaches this
    # function three times, so the economy is bound by Builder-turns per
    # deposit. Anchoring the order on the Core and the belt we already own is
    # the one ordering that minimises those turns directly.
    anchors = set(p.foot) | set(p.network_tiles) | set(p.conveyors)
    if not anchors and p.core:
        anchors = {p.core}
    def _to_network(ore):
        return min(max(abs(ore[0] - a[0]), abs(ore[1] - a[1])) for a in anchors)
    candidates = sorted(
        p.ores - claimed,
        key=(_to_network if ANCHOR_ORE_ON_NETWORK and anchors
             else (lambda ore: max(abs(ore[0] - me[0]), abs(ore[1] - me[1])))),
    )
...
def _build_basic_gunner(p, ct, enemy_core):
    """Seat the siege battery. Sentinel first -- see SIEGE_SENTINEL_FIRST_TRY."""
    if SIEGE_SENTINEL_FIRST_TRY and _build_siege_sentinel(p, ct, enemy_core):
        return True
    return _build_basic_gunner_body(p, ct, enemy_core)


def _build_basic_gunner_body(p, ct, enemy_core):
