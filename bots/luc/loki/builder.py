...
def _sense(p, ct):
    for tile in ct.get_nearby_tiles():
        ...
        if bid is None:
            # loki: a tile that held an enemy conveyor and is now empty is a
            # cut — theirs to re-lay for 3 Ti unless we wall it first. The
            # wall costs the same 3 Ti and ends the argument.
            if key in p.enemy_conveyors:
                p.broken_belt.add(key)
...
def _build_failure(p, ct, target, kind, cost, allow_ore=False):
    ...
    if reason_code in ("resources", "cooldown"):
        # ...but not forever. Under sustained fire the ammo emergency pins the
        # bank at EMERGENCY_RESERVE every round, and a build that waits on
        # titanium holds its claim slot — one of two — and its Builder for
        # the rest of the game (watched: 160 rounds of prelay on jackpot
        # while unclaimed ore sat everywhere). Since any friendly belt is
        # joinable now, abandoning is cheap: a later claim re-prices this
        # deposit with the half-built line as a shortcut.
        return p.build_wait_rounds > RESOURCE_WAIT_ABANDON_ROUNDS
    return True
...
def _harass(p, ct):
    ...
    if exposed:
        ...
        if seat is not None:
            ...
            return
    # loki: wall the cut before making the next one. A broken belt tile they
    # can re-lay for 3 Ti is a wound that heals; the same 3 Ti as a barrier
    # is permanent, and every Harvester upstream of it mines into a dead end
    # for the rest of the game.
    if p.broken_belt:
        cut = min((t for t in p.broken_belt), key=lambda t: (
            max(abs(t[0] - me[0]), abs(t[1] - me[1])), t))
        if (t := cut) and t not in p.walls and t not in p.solids:
            if _cardinal_distance(me, cut) == 1:
                if (ct.get_global_resources() >= ct.get_barrier_cost()
                        + SIEGE_BARRIER_RESERVE
                        and ct.can_build_barrier(Position(*cut))):
                    ct.build_barrier(Position(*cut))
                    _mark_progress(p, ct, "walled enemy belt cut", cut)
                    p.solids.add(cut)
                    p.broken_belt.discard(cut)
                    return
            elif max(abs(cut[0] - me[0]), abs(cut[1] - me[1])) <= 6:
                _step(p, ct, Position(*cut), True)
                return
        else:
            p.broken_belt.discard(cut)
...
def _wall_in_enemy(p, ct, enemy_core):
    """Ring the enemy base with barriers, minus one door, then cover the door.

    The mirror of our own seal, aimed the other way. A finished ring means no
    new Builder, Harvester or conveyor leaves their spawn area except through
    the door — and the door is chosen to be the tile their economy has to
    use (nearest the known ore), covered by _gap_sentinel from a seat their
    Core-damage logic never flags.
    """
    if (not SIEGE_WALL_ENABLED or enemy_core is None
            or ct.get_current_round() < SIEGE_WALL_ROUND):
        return False
    if not hasattr(p, "wallin_targets"):
        foot = {(enemy_core[0] + dx, enemy_core[1] + dy)
                for dx in (0, 1) for dy in (0, 1)}
        x0 = min(t[0] for t in foot) - SIEGE_WALL_RADIUS
        x1 = max(t[0] for t in foot) + SIEGE_WALL_RADIUS
        y0 = min(t[1] for t in foot) - SIEGE_WALL_RADIUS
        y1 = max(t[1] for t in foot) + SIEGE_WALL_RADIUS
        edge = ([(x, y0) for x in range(x0, x1 + 1)]
                + [(x1, y) for y in range(y0 + 1, y1 + 1)]
                + [(x, y1) for x in range(x1 - 1, x0 - 1, -1)]
                + [(x0, y) for y in range(y1 - 1, y0, -1)])
        ring = [t for t in edge
                if _inside(p, t) and t not in p.walls and t not in p.ores]
        if len(ring) < 4:
            p.wallin_targets, p.wallin_gap = [], None
            return False
        if p.ores:
            cx = sum(t[0] for t in p.ores) / len(p.ores)
            cy = sum(t[1] for t in p.ores) / len(p.ores)
            gap = min(ring, key=lambda t: ((t[0] - cx) ** 2
                                           + (t[1] - cy) ** 2, t))
        else:
            gap = min(ring, key=lambda t: (_distance_sq(t, p.core), t))
        p.wallin_gap = gap
...
def _gap_sentinel(p, ct, enemy_core):
    """Cover the door with a Sentinel whose ray never reaches their Core.

    The seat constraint is the whole trick: a defence that keys on Core
    damage (ours does — the shooter beacon publishes Core-range turrets and
    nothing else) cannot see a turret that only ever kills Builders crossing
    one wall tile.
    """
    if not GAP_SENTINEL or getattr(p, "gap_sentinel_built", False):
        return False
    gap = p.wallin_gap
    if gap is None or ct.get_global_ammo() < MIN_AMMO_FOR_SENTINEL:
        return False
    foot = {(enemy_core[0] + dx, enemy_core[1] + dy)
            for dx in (0, 1) for dy in (0, 1)}
    choices = []
    for dx in (-1, 0, 1):
        for dy in (-1, 0, 1):
            if dx == 0 and dy == 0:
                continue
            for k in range(2, 6):
                spot = (gap[0] + dx * k, gap[1] + dy * k)
                if (not _inside(p, spot) or spot in p.walls
                        or spot in p.solids or spot in p.ores
                        or spot in p.foot or spot not in p.seen
                        or _distance_sq(spot, gap) > SENTINEL_RANGE_SQ):
                    continue
                facing = _ray_direction(spot, gap)
                if facing is None:
                    continue
                if any(_ray_direction(spot, f) == facing
                       and _distance_sq(spot, f) <= SENTINEL_RANGE_SQ
                       for f in foot):
                    continue
                if not ct.can_fire_from(Position(*spot), facing,
                                        EntityType.SENTINEL, Position(*gap)):
                    continue
                choices.append((-min(_distance_sq(spot, f) for f in foot),
    ...
    if _cardinal_distance(me, spot) != 1:
        _move_cardinal_adjacent(p, ct, spot)
        return True
    if ct.can_build_sentinel(Position(*spot), facing):
        ct.build_sentinel(Position(*spot), facing)
        _mark_progress(p, ct, "door sentinel", spot)
        p.solids.add(spot)
        p.gap_sentinel_built = True
        print(f"DOOR round={ct.get_current_round()} gap={gap} seat={spot}",
              file=sys.stderr, flush=True)
    elif _build_failure(p, ct, spot, "door sentinel", ct.get_sentinel_cost()):
        pass
    return True
...
def _rush(p, ct):
    ...
    if _block_siege_lane(p, ct):
        return
    # loki: the wall outranks the next turret. A ring that closes ends their
    # economy outright, where a turret trades shots with a mender.
    if _wall_in_enemy(p, ct, enemy_core):
        return
