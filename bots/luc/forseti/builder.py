...
from constants import (
    ...
    SENTINEL_SIEGE_FIRST,
    SIEGE_SEARCH_EVERY,
    SIEGE_SENTINEL_FROM_ROUND,
...
def _build_siege_sentinel(p, ct, enemy_core):
    ...
    # Ammo first, throttle second: the old order burned the 10-round search
    # slot on a pool that happened to be low, so a chronically firing team
    # searched for a seat almost never — 0.61 Sentinels a game against a
    # target of 2. The ammo test costs one engine call; the throttle exists
    # for the 13x13 search below it.
...
def _build_basic_gunner(p, ct, enemy_core):
    ...
    # SENTINEL_SIEGE_FIRST used to be a label, not a behavior: the Sentinel
    # ran only after the Gunner search came up empty, so on any map with a
    # legal Gunner lane the battery was Gunners — which die. Watched on vault
    # vs vidar: our five Gunners (all dead by round 70) dealt 1449 Core damage
    # over the game against 3960 from vidar's four Sentinels, which shot from
    # range through terrain and never took a hit. The per-titanium arithmetic
    # that ordered Gunners first prices damage but not survival; a turret that
    # dies after three shots delivers neither.
    # ...but not in the opening: the reorder tried unconditionally lost to
    # vidar's rush by round 89 — the attacker paused to seat 40-ammo Sentinels
    # while the enemy walked over an unpressured board. Gunner tempo opens;
    # the Sentinel battery is the mid-game conversion.
    if (SENTINEL_SIEGE_FIRST
            and ct.get_current_round() >= SIEGE_SENTINEL_FROM_ROUND
            and _build_siege_sentinel(p, ct, enemy_core)):
        return True
    # While a Sentinel seat is still owed and the bank can cover the ammo
    # gap, a 25-ammo Gunner here is what keeps the pool under the Sentinel's
    # 40 forever: the Core converts toward AMMO_TARGET every round, and this
    # build is the drain that outruns it. Hold — the caller harasses instead,
    # which spends nothing.
    if (SENTINEL_SIEGE_FIRST
            and ct.get_current_round() >= SIEGE_SENTINEL_FROM_ROUND
            and p.siege_sentinels_built < SIEGE_SENTINEL_TARGET
            and ct.get_global_ammo() < MIN_AMMO_FOR_SENTINEL
            and ct.get_global_resources() >= (MIN_AMMO_FOR_SENTINEL
                                              - ct.get_global_ammo() + 10)):
        return False
    if ct.get_global_ammo() < MIN_AMMO_FOR_GUNNER:
        return False
    core_tiles = {(enemy_core[0] + dx, enemy_core[1] + dy)
