...
def _build_home_guard(p, ct, enemy_core):
    """Seat one Gunner on the ring, facing the way the enemy has to come.

    Deliberately independent of seeing an enemy: _defend_core needs a visible
    target to align with and only runs once the Core is already being hit,
    which is far too late when the enemy's turret arrives on round 10.
    """
    for target in _launcher_ring_targets(p, enemy_core):
        spot = tuple(target)
        if spot in p.solids or spot in p.walls or spot in p.ores:
            continue
        if ct.is_in_vision(target) and ct.get_tile_building_id(target) is not None:
            continue
        # Face the approach, not the Core itself. _ray_direction needs exact
        # eight-way alignment, which a ring site essentially never has, so
        # using it here disabled the guard permanently on the first attempt.
        dx = _sign(enemy_core[0] - spot[0])
        dy = _sign(enemy_core[1] - spot[1])
        facing = next((d for d in D8 if d.delta() == (dx, dy)), None)
        if facing is None:
            continue
        here = tuple(ct.get_position())
        if _cardinal_distance(here, spot) != 1:
            _move_cardinal_adjacent(p, ct, spot)
            return True
        if (ct.get_global_resources() >= ct.get_gunner_cost()
                and ct.can_build_gunner(target, facing)):
            ct.build_gunner(target, facing)
            _mark_progress(p, ct, "built home guard", spot)
            p.solids.add(spot)
            p.home_guard_built = True
            p.home_gunners_built += 1
            return True
        return False
    p.home_guard_built = True  # nowhere to put one; stop trying
    return False
...
def _run_launcher_ring(p, ct):
    ...
    if p.launcher_ring_built >= RING_MAX_SITES:
        return True
    # The pad is up; make the *next* ring site a Gunner instead of a second
    # Launcher. Measured across 84 gate games: in wins our first home turret
    # lands on round 14 and in losses on round 20, while the enemy plants a
    # turret beside our Core on round 10 either way -- and in 12 of 38 losses
    # we never build a home turret at all. A Launcher there is a displacement
    # screen that cannot shoot; a Gunner is the thing the timing says decides
    # the game. Same +10% cost scale either way.
    if (HOME_GUARD_AFTER_PAD and p.launcher_ring_built >= 1
            and not p.home_guard_built
            and ct.get_global_ammo() >= MIN_AMMO_FOR_GUNNER):
        if _build_home_guard(p, ct, enemy_core):
            return False
