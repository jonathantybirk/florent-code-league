...
def _keep_ammunition(ct, extra_reserve: int = 0) -> None:
    ...
    try:
        ...
        if held < COMBAT_AMMO_FLOOR:
            # fulla: past the opening, the emergency keeps belt money. The
            # old floor pinned the bank at 10 Ti for entire attrition games,
            # so a 3 Ti conveyor twenty tiles from the fight waited forever
            # and the economy that pays for the ammunition never grew. Two
            # conveyors' worth is not what loses a firefight; a two-Harvester
            # economy at round 400 is.
            floor = EMERGENCY_RESERVE
            if ct.get_current_round() >= 120:
                floor = max(EMERGENCY_RESERVE, 2 * ct.get_conveyor_cost())
            amount = max(amount, min(
                COMBAT_AMMO_FLOOR - held,
                ct.get_global_resources() - floor,
            ))
