...
# byggvir: 7 -> 999. Pivot (rank 7) fields ZERO Launchers with the same
# Gunner count we run and three times the economy; every Launcher we build is
# titanium and cost-scale their Harvesters get for free. A stop distance no
# map exceeds turns the whole ferry/pad apparatus off without touching its
# code.
RELAY_STOP_DISTANCE = 999
...
def _run_launcher_ring(p, ct):
    ...
    # visible on it.
    ordered = (p.launcher_ring_targets[p.ring_slot::max(1, LAUNCHER_BUILDERS)]
