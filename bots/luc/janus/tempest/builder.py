...
def _opening_ferry(p, ct, enemy_core):
    ...
    The atlas used to be the only thing that made the Core's location certain
    enough to ferry towards, so this returned early without one. It no longer
    has to: the only caller is _rush, which fires only once SLOT_ENEMY_CORE is
    set, and that happens when a Builder has either seen the Core or reduced
    the three symmetry hypotheses to one. Both are proofs, not guesses.
    """
