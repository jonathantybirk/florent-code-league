...
# ...and now zero, which is what the live replays argue for. Decoded over 25
# ladder games, our first Launcher lands on **round 3** and appears in 17 of 30
# sides; our opponents' first lands on round 180 and appears in 4 of 30. A
# Launcher deals no damage at all -- it repositions a Builder -- so this is 20
# titanium plus +10% on every later price, spent in the compounding window, on
# a tool essentially nobody else on the ladder buys.
#
# The ring *Builder* is untouched: it is set from doctrine.launcher_builders,
# not from this constant, so it still takes its post and still mends the Core,
# which one-flag ablation prices at 11.0pp. Only the pad it would have built is
# gone.
RING_MAX_SITES = 0
