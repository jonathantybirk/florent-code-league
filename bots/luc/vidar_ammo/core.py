...
from constants import (HEARTBEAT_MASK, HEARTBEAT_SHIFT,
                       ...
                       PAD_FIRST_ORDER,
                       REINFORCE_RESERVE, ECON_EXPAND_BUILDERS,
                       ECON_BUILDER_ROUND, ECON_MAX_LIVE_BUILDERS,
                       ECON_MAX_TOTAL_BUILDERS, ECON_EXPAND_RESERVE,
                       ECON_EXPAND_FLAG,
...
# Ammunition is denominated in shots, not in titanium, and 2.3.4 repriced the
# shot. Both of these numbers were set when a Gunner fired for 2 ammunition:
# 120 held sixty shots and the 80 floor held forty. The patch took the Gunner
# to 4 and this bot's default turret to the Sentinel's 10, so the same two
# numbers now hold twelve shots and eight -- a fivefold cut in the only unit
# that means anything, arriving silently as a side effect of a balance patch.
#
# Re-derived at 10 ammunition a shot, the target should hold twenty Sentinel
# shots and so be raised to 200. MEASURED AND REJECTED: it costs 43% of the
# economy. On quarry against odin, target 200 mines 11,410 titanium and target
# 120 mines 20,180 in the same deterministic game.
#
# The reasoning that produced 200 was right about ammunition and wrong about
# what the number does. `_keep_ammunition` converts everything above the
# construction reserve until the pool reaches the target, and that reserve is
# only max(60, harvester, launcher) -- so a high target does not buy a
# stockpile out of surplus, it pins every Builder's working bank at the
# reserve for the whole game, and a belt that needs six scaled conveyors and a
# Harvester in hand can never afford them at once. Ammunition sitting in the
# pool is titanium not in the ground.
#
# 120 stays. The Sentinel's appetite is real but it is a *rate*, and the answer
# to a rate is income, not a buffer.
AMMO_TARGET = 120
...
#
# Deliberately NOT raised with AMMO_TARGET, and the first draft of this bot
# raising it to 100 is why the line is here. This floor overrides the
# construction reserve and drains the bank to EMERGENCY_RESERVE to reach it.
# A Gunner army spends 2 Ti a round, so the pool sat above 80 and the override
# fired rarely; a Sentinel army spends 3.33 and sits *under* any floor set near
# its own burn rate, so the override fires every round and the Core hands the
# builders a bank of 10 Ti for the whole game. Traced on quarry against odin:
# 11,410 titanium mined against 17,150 and 107 buildings against 183 -- the
...
def run(player: "Player", ct: Controller) -> None:
    ...
    # Raised on the round the expansion regime opens, not on the round a
    # Builder is spawned into it: store writes are visible only next round, and
    # the Builder reads its role on its own first turn. Setting it early costs
    # nothing and setting it late gives the new miner an attacker's job.
    if (ECON_EXPAND_BUILDERS
            and ct.get_current_round() >= ECON_BUILDER_ROUND
            and not getattr(player, "core_dying", False)):
        alarm |= ECON_EXPAND_FLAG
    ...
    if role >= MAX_OPENING_BUILDERS:
        # Past the opening this Core only ever replaced the dead: a Builder
        # arrives because the titanium stopped or because the Core projects its
        # own death. Both are emergencies, and neither is what a long game is
        # actually decided by.
        #
        # 2.3.4 made that a hole. Defence is about 2.2x more titanium-efficient
        # than offence -- a mender restores 4 HP for a flat 1 Ti against a
        # Sentinel's 6 damage for 3.33 -- so Cores mostly do not die and the
        # median game reaches round 1000, where the first tiebreak criterion is
        # titanium collected. In that game a fourth Builder laying belt is not
        # a body in an attrition war, it is income.
        #
        # The headcount-of-three finding this overrides was about the *opening*
        # and it still holds: an opening Builder is +20% on the Launchers and
        # Harvesters the opening has not bought yet. Levied at round 250 the
        # same +20% falls almost entirely on conveyors, which are 3 Ti -- so
        # the tax is 0.6 Ti a tile, against the 2.5 Ti a round a connected
        # Harvester returns for the remaining three quarters of the game.
        expanding = (
        ...
        if not (getattr(player, "income_dead", False)
                ...
                or expanding):
            return
        live_cap = ECON_MAX_LIVE_BUILDERS if expanding else MAX_LIVE_BUILDERS
        total_cap = ECON_MAX_TOTAL_BUILDERS if expanding else MAX_TOTAL_BUILDERS
        if live >= live_cap or role >= total_cap:
            return
        if not expanding and resources < builder_cost + REINFORCE_RESERVE:
            return
...
def _keep_ammunition(ct) -> None:
    ...
    try:
        ...
        if held < COMBAT_AMMO_FLOOR:
            # Never convert away the price of a Harvester.
            #
            # This branch overrides the construction reserve and drains the
            # bank to EMERGENCY_RESERVE, which was fine when a Gunner army
            # spent 2 Ti a round and the pool sat above the floor most of the
            # time. A Sentinel army spends 3.33 and therefore sits *under* any
            # floor near its own burn rate, so the override fires every round
            # and pins the team's working bank at 10 Ti for the whole game.
            #
            # Traced on twins against prospect: global titanium 10, 10, 7, 9, 7
            # from round 30 to round 70 with one Harvester on the board, the
            # Core falling 500 -> 81, the guard frozen in `prelay` and the
            # miner frozen in `goto` -- neither able to afford a 30 Ti Sentinel,
            # a 20 Ti Gunner or the 20 Ti Harvester that would have ended the
            # famine. Ammunition is worthless to a team with no economy to
            # sustain it, and a Harvester is the one purchase that repays the
            # titanium it costs.
            floor_reserve = max(EMERGENCY_RESERVE, ct.get_harvester_cost())
            amount = max(amount, min(
                COMBAT_AMMO_FLOOR - held,
                ct.get_global_resources() - floor_reserve,
            ))
