...
# ...but "this close" was closer than a Sentinel can shoot.
#
# A Sentinel reaches r^2=32 and a siege Sentinel is deliberately emplaced at the
# far end of that, where the Core's own defenders cannot reach it. The mender
# standing on the Core is therefore at ~32 -- outside this radius -- so the
# Sentinel skips rule 1 and shoots the Core instead, which is exactly the
# "paying to lose slowly" the comment above warns about.
#
# Measured in live replays: two of five games lost to Pivot had our Core
# untouched while we dealt **2828 and 4123 damage** to theirs without killing a
# 500-HP Core, both running the full 1000 rounds and lost on the titanium
# tiebreak. That is the mender arithmetic playing out exactly as written, in the
# one situation the constant was too small to cover.
