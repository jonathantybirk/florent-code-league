...
from constants import (
    ...
    SLOT_OWN_CORE,
    SENTINEL_RANGE_SQ,
    SENTINEL_DAMAGE_PER_ROUND,
    GUNNER_DAMAGE_PER_ROUND,
    FIRE_SURVIVAL_MARGIN,
...
# Buildings of ours a Builder may demolish to get its own work done.
REMOVABLE_OWN = (EntityType.GUNNER, EntityType.SENTINEL,
                 EntityType.LAUNCHER, EntityType.BARRIER)
GUNNER_RANGE_SQ = 13
...
def _run(p, ct):
    ...
    if not hasattr(p, "builder_index"):
        ...
        p.network_tiles = set()
        p.network_plan = {}
        p.harvesters, p.friendly_launchers = set(), set()
        p.own_turrets = set()
        p.enemy_turrets = {}
        p.enemy_fire = {}
        p.network_load = 0
    ...
    _sense(p, ct)
    p.hp = ct.get_hp(ct.get_id())
    _refresh_enemy_fire(p)
    _report_stall(p, ct)
    if p.atlas is None:
        _update_enemy_core_inference(p, ct)
    if p.core is None:
        return
    _adopt_orphaned_ring(p, ct)
    if _write_off(p, ct):
        return
...
def _adopt_orphaned_ring(p, ct):
    """Let a replacement Builder take over an unfinished ring.

    Roles come from a monotonic ticket, so when the ring Builder dies its
    replacement draws the next index and becomes an attacker. The ring is then
    orphaned wherever it got to -- measured at three of eight sites, which is
    why the band around the base kept having holes in it. Any Builder spawned
    after the opening checks whether the ring still has gaps and, if so, takes
    the job instead of walking off to attack.
    """
    if getattr(p, "role_settled", False) or p.builder_index < MAX_OPENING_BUILDERS:
        return
    p.role_settled = True
    packed = ct.read_store(SLOT_ENEMY_CORE)
    if not packed:
        p.role_settled = False      # no ring plan yet; decide next round
        return
    for target in _launcher_ring_targets(p, unpack_pos(packed)):
        if tuple(target) in p.solids:
            continue
        if ct.is_in_vision(target) and ct.get_tile_building_id(target) is not None:
            continue
        p.is_launcher_builder = True
        p.is_attacker = False
        p.ring_slot = 0
        return
...
def _sense(p, ct):
    for tile in ct.get_nearby_tiles():
        ...
        if bid is None:
            ...
            p.harvesters.discard(key)
            p.friendly_launchers.discard(key)
            p.own_turrets.discard(key)
        ...
        if enemy and kind in HARASS_PRIORITY:
            ...
            p.enemy_economy.pop(key, None)
        if kind == EntityType.HARVESTER and not enemy:
            p.harvesters.add(key)
        else:
            p.harvesters.discard(key)
        if kind == EntityType.LAUNCHER and not enemy:
            p.friendly_launchers.add(key)
        else:
            p.friendly_launchers.discard(key)
        if not enemy and kind in REMOVABLE_OWN:
            p.own_turrets.add(key)
        else:
            p.own_turrets.discard(key)
...
def _pick(p, ct):
    ...
    if p.network_load >= 4:
        # A conveyor carries one stack per round, which is exactly four
        # Harvesters at their cadence. A fifth mines into a queue and pays for
        # nothing, so start a fresh line to the Core instead of stopping.
        p.network_tiles = set()
        p.network_load = 0
    ...
    # Rank by how far the Builder actually has to walk, not by how far the ore
    # looks. Straight-line order sent it around walls to a deposit that was
    # nearer on paper and half a map away on foot; one distance map costs a
    # single search and orders every candidate honestly.
    reach = _distance_map(p, me)
    candidates = []
    for ore in p.ores - claimed:
        travel = min((reach[goal] for goal in _adjacent(p, ore) if goal in reach),
                     default=None)
        if travel is not None:
            candidates.append((travel, ore))
    candidates.sort()
    for travel, ore in candidates:
        route = _route(p, ore)
        if route is None:
            continue
        best = (travel, len(route), ore, route)
        break
    if best is None and p.economy_lines_completed:
        # Nothing left this Builder can reach and feed. Standing at a finished
        # network is worth less than another body at the front.
        p.is_attacker = True
        p.phase = "rush"
        return
    if best is None:
...
def _trunk_terminal(p, tile):
    """Follow a conveyor downstream to the last tile before the Core."""
    seen = set()
    current = tile
    while current in p.conveyors and current not in seen:
        seen.add(current)
        dx, dy = p.conveyors[current].delta()
...
def _trunk_load(p):
    """Harvesters already feeding each line, keyed by the tile that feeds the Core.

    A conveyor moves one stack a round, which is four Harvesters at their
    cadence, so what matters is not how many lines this Builder personally laid
    but how many Harvesters share the tile where a line meets the Core. Counted
    from the map rather than from private bookkeeping, which is what lets a
    Builder join a line somebody else built.
    """
    load = {}
    for harvester in p.harvesters:
        for dx, dy in D4_DELTAS:
            neighbour = (harvester[0] + dx, harvester[1] + dy)
            if neighbour in p.foot:
                break            # feeds the Core directly, costs no line
            if neighbour in p.conveyors:
                terminal = _trunk_terminal(p, neighbour)
                if terminal is not None:
                    load[terminal] = load.get(terminal, 0) + 1
                break
    return load
...
def _route(p, ore):
    ...
    # Any conveyor of ours is fair game, not merely the ones this Builder laid,
    # provided the line it belongs to is not already carrying its four
    # Harvesters. Refusing to reuse other people's conveyor was why a second
    # Builder would run a parallel line beside an existing one.
    load = _trunk_load(p)
    joinable = {tile for tile in p.conveyors
                if load.get(_trunk_terminal(p, tile), 0) < 4}
    # Worth revisiting later: this is a greedy join, not an optimal layout. A
    # long spur to a distant deposit looks like it serves one Harvester, when
    # the same spur could carry four if the next three deposits out there were
    # planned onto it -- and conversely, a line that has already picked up
    # nearby deposits has little capacity left to justify extending it far.
    # Solving that properly means routing several deposits at once at game
    # time, bounded to the few we could realistically reach, and probably wants
    # a cheap "cut here and re-route" primitive so a layout can be revised
    # rather than rebuilt. Not obviously worth it before the basics are solid.
    ...
    if ct.can_build_conveyor(target, facing):
        ...
        if not compatible:
            if (building_id is not None
                    and ct.get_team(building_id) != ct.get_team()
                    and _break_route_blocker(p, ct, tile)):
                return
...
def _break_route_blocker(p, ct, tile):
    """Put a Gunner on an enemy building sitting across our conveyor route.

    Rerouting around enemy infrastructure concedes the ground it was placed to
    take, and on a tight map the reroute may not exist. Shooting it clears the
    original line and costs them the building.
    """
    ...
    for direction in D8:
        spot = me.add(direction)
        key = tuple(spot)
        if (not _inside(p, key) or key in p.foot or key in p.walls
                or key in p.solids or key in p.ores):
            continue
        facing = _ray_direction(key, tile)
        if (facing is None
                or _distance_sq(key, tile) > GUNNER_RANGE_SQ
                or not ct.can_fire_from(spot, facing, EntityType.GUNNER, target)
                or not _preserves_friendly_turret_lanes(ct, spot, protected)
                or not ct.can_build_gunner(spot, facing)):
            continue
        ct.build_gunner(spot, facing)
        _mark_progress(p, ct, "built route breaker", key)
        p.solids.add(key)
        return True
    return False
...
def _mark_progress(p, ct, action, target=None):
    # Any progress at all clears the stuck counter. Without this it only ever
    # went up: a Builder that laid conveyor all game but failed to path now and
    # then still crossed the write-off threshold eventually and destroyed
    # itself for no reason a replay could show.
    p.path_failures = 0
    p.path_failure_round = -1
    p.bot_wait = 0
    p.last_progress_round = ct.get_current_round()
...
def _step(p, ct, target, exact, allow_launcher=True):
    ...
    if _hold_for_bots(p, ct, tuple(source), tuple(target), exact):
        return True
    ...
    # Once per round, not once per call. _step runs several times in a turn --
    # 25 times in round one, measured -- so a raw counter reached the write-off
    # threshold of 40 within a few rounds and destroyed healthy Builders on
    # frame 4. Everything reading this counter means rounds, including
    # BLOCKED_ROUNDS_BEFORE_LAUNCHER.
    if _clear_own_blocker(p, ct, tuple(source), target, exact):
        return True
    round_now = ct.get_current_round()
    if p.path_failure_round != round_now:
        p.path_failure_round = round_now
        p.path_failures += 1
...
def _build_escape_launcher(p, ct, target):
    ...
    here = tuple(ct.get_position())
    # A Launcher built purely to escape with is 20 Ti and +10% spent on one
    # trip. If one of the ring's own sites is reachable it does both jobs, so
    # prefer it even when it is the longer throw.
    if not hasattr(p, "ring_sites"):
        packed = ct.read_store(SLOT_ENEMY_CORE)
        p.ring_sites = ({tuple(site) for site in
                         _launcher_ring_targets(p, unpack_pos(packed))}
                        if packed else set())
    # Ranking the four tiles underfoot was not enough: at round 2 the attacker
    # stands beside the Core and none of them is a ring site, so every escape
    # Launcher landed off-ring -- nought out of eight, measured. Walk to a ring
    # site instead when one is near enough to be worth the detour.
    ring_options = [site for site in p.ring_sites
                    if site not in p.solids and site not in p.walls
                    and site not in p.ores
                    and _cardinal_distance(here, site) <= ESCAPE_RING_DETOUR]
    if ring_options:
        site = min(ring_options,
                   key=lambda s: (_cardinal_distance(here, s),
                                  _distance_sq(s, tuple(target)), s))
        if _cardinal_distance(here, site) != 1:
            _move_cardinal_adjacent(p, ct, site)
            return True
        position = Position(*site)
        if ct.can_build_launcher(position):
            ct.build_launcher(position)
            _mark_progress(p, ct, "built escape launcher", site)
            p.solids.add(site)
            p.path_failures = 0
            p.awaiting_launch = LAUNCH_REQUEST_ROUNDS
    ...
    for dx, dy in D4_DELTAS:
        ...
        if (_inside(p, spot) and spot not in p.walls and spot not in p.solids
            ...
            candidates.append((spot not in p.ring_sites,
                               position.distance_squared(target), spot, position))
    if not candidates:
        _plan_failed(
            p, ct, "build escape launcher", target,
    ...
    _, _, spot, position = min(candidates)
    ct.build_launcher(position)
...
def _announce_launch(p, ct, target, launcher_position):
    """Publish this passenger and where it is actually trying to get to."""
    packed = pack_pos((target.x, target.y))
    if packed > LAUNCH_TARGET_MASK:
        _plan_failed(p, ct, "announce launch", target,
                     "destination does not fit the request slot")
        return False
    slot = LAUNCH_REQUEST_SLOTS[p.builder_index % len(LAUNCH_REQUEST_SLOTS)]
    ct.write_store(slot, (ct.get_id() << LAUNCH_TARGET_BITS) | packed)
    return True
...
def _bfs_step(p, source, target, exact, avoid_launchers=True):
    ...
    Clean ground first. If the only way through is under fire, take it anyway
    provided the trip would not kill us -- arriving three rounds sooner is
    usually worth ten health, and refusing every lane leaves the attacker
    circling a base it was sent to break.
    """
    path = _bfs_path(p, source, target, exact, avoid_launchers=avoid_launchers)
    if path is None and p.enemy_fire:
        risky = _bfs_path(p, source, target, exact,
                          avoid_launchers=avoid_launchers, ignore_fire=True)
        survivable = getattr(p, "hp", 0) - FIRE_SURVIVAL_MARGIN
        if risky is not None and _route_damage(p, risky) < survivable:
            path = risky
...
def _refresh_enemy_fire(p):
    """Map every tile a known enemy turret can shoot to the damage it takes.

    Damage, not a ban. A lane is a cost to weigh: crossing one tile of Gunner
    fire is ten health, which a healthy Builder can spend to arrive several
    rounds sooner. Only a route that would actually kill it is impassable.

    A Gunner's line stops at the first wall; a Sentinel's does not stop at all,
    which is why it is traced to full range regardless.
    """
    fire = {}
    for position, (kind, direction) in p.enemy_turrets.items():
        if kind == EntityType.SENTINEL:
            reach, damage, blockable = SENTINEL_RANGE_SQ, SENTINEL_DAMAGE_PER_ROUND, False
        else:
            reach, damage, blockable = GUNNER_RANGE_SQ, GUNNER_DAMAGE_PER_ROUND, True
        dx, dy = direction.delta()
        if dx == 0 and dy == 0:
            continue
        step = 1
        while True:
            tile = (position[0] + dx * step, position[1] + dy * step)
            if not _inside(p, tile) or _distance_sq(tile, position) > reach:
                break
            if blockable and tile in p.walls:
                break
            fire[tile] = fire.get(tile, 0) + damage
            step += 1
    p.enemy_fire = fire


def _route_damage(p, path):
    """Health a route costs, one round of exposure per tile crossed."""
    return sum(p.enemy_fire.get(tile, 0) for tile in path)


def _bfs_path(p, source, target, exact, avoid_launchers=True,
              extra_blocked=(), ignore_bots=False,
              ignore_own_turrets=False, ignore_fire=False):
    """Full cardinal route from source to a goal tile, or None."""
    goals = {target} if exact else _adjacent(p, target)
    if source in goals:
        return [source]
    blocked = p.walls | p.foot | p.solids
    if not ignore_fire and p.enemy_fire:
        blocked = blocked | (set(p.enemy_fire) - {source})
    if ignore_own_turrets:
        blocked = blocked - p.own_turrets
    if not ignore_bots:
        blocked = blocked | (p.bot_occupied - {source})
    if avoid_launchers:
...
def _cached_path(p, ct, source, target, exact, ignore_bots=False):
    """One search per distinct question per turn.

    _step runs many times a turn, so anything asking the pathfinder a question
    inside it has to memoise or the turn blows its 10 ms budget.
    """
    round_now = ct.get_current_round()
    if getattr(p, "path_memo_round", None) != round_now:
        p.path_memo_round = round_now
        p.path_memo = {}
    key = (source, target, exact, ignore_bots)
    if key not in p.path_memo:
        p.path_memo[key] = _bfs_path(p, source, target, exact,
                                     ignore_bots=ignore_bots)
    return p.path_memo[key]


def _hold_for_bots(p, ct, source, target, exact):
    """Stand still rather than walk around a Builder that is about to move.

    Patience is scaled to what impatience would cost. A couple of extra steps
    is cheaper than a wasted tick, so that detour is simply taken. A long way
    round is worth one tick of waiting. No way round at all means the route --
    and with it the Harvester site at the end of it -- has to be replanned, and
    that is worth holding several ticks for.
    """
    clear = _cached_path(p, ct, source, target, exact, ignore_bots=True)
    if clear is None:
        return False                      # not a bot in the way; nothing to wait on
    around = _cached_path(p, ct, source, target, exact)
    if around is not None and len(around) - len(clear) <= BOT_DETOUR_TOLERANCE:
        p.bot_wait = 0
        return False                      # cheap enough, just go round
    limit = BOT_WAIT_REPLAN if around is None else BOT_WAIT_DETOUR
    if getattr(p, "bot_wait", 0) >= limit:
        return False                      # waited long enough; replan
    p.bot_wait = getattr(p, "bot_wait", 0) + 1
    return True


def _clear_own_blocker(p, ct, source, target, exact):
    """Demolish one of our turrets that is now only blocking our own work.

    A Gunner thrown up to answer a raid earns its tile while the raid is on.
    Once the attackers are gone it is a wall across our own ground, and the ore
    behind it stops being mined at all -- silently, because the Builder just
    routes elsewhere or gives up. Cost scaling makes it worse than neutral: the
    turret keeps charging its +10% on every build for as long as it stands.

    Only with nothing hostile in sight, and never a ring site, which is the
    defence rather than a leftover. Demolishing an allied building costs no
    action cooldown, so this is close to free where it applies.
    """
    if not p.own_turrets:
        return False
    if any(ct.get_team(entity) != ct.get_team()
           for entity in ct.get_nearby_entities()):
        return False
    if _bfs_path(p, source, tuple(target), exact) is not None:
        return False                      # not our own building in the way
    without = _bfs_path(p, source, tuple(target), exact,
                        ignore_own_turrets=True)
    if without is None:
        return False                      # something else blocks it too
    ring = getattr(p, "ring_sites", frozenset())
    for tile in without:
        if tile not in p.own_turrets or tile in ring:
            continue
        if _cardinal_distance(source, tile) != 1:
            _move_cardinal_adjacent(p, ct, tile)
            return True
        position = Position(*tile)
        if not ct.can_destroy(position):
            continue
        ct.destroy(position)
        _mark_progress(p, ct, "cleared our own turret", tile)
        p.solids.discard(tile)
        p.own_turrets.discard(tile)
        return True
    return False
...
def _run_launcher_ring(p, ct):
    ...
    if not hasattr(p, "launcher_ring_targets"):
        ...
        p.ring_terrain = -1
    # Re-solve as terrain is learned. Nothing here is a lookup: the cover is
    # computed from the Core, the map bounds and whatever walls and ore are
    # known so far. On a map the atlas does not carry, round-three knowledge is
    # mostly blank, which makes the first plan needlessly wide -- unknown
    # ground reads as open, so it asks for Launchers facing directions that
    # turn out to be solid rock. Re-solving whenever the terrain picture grows
    # lets the ring tighten instead of paying for that first guess all game.
    terrain = len(p.walls) + len(p.ores)
    if terrain != p.ring_terrain:
        p.ring_terrain = terrain
        p.launcher_ring_targets = _launcher_ring_targets(
            p, enemy_core, p.ring_blocked)
    ...
    for target in ordered:
        ...
        if key in p.launcher_ring_done:
            # A site stays done only while something of ours is standing on it.
            # Once shot, the ring has a hole exactly where the enemy just
            # proved it wants to walk, so put it back.
            if ct.is_in_vision(target) and ct.get_tile_building_id(target) is None:
                p.launcher_ring_done.discard(key)
                p.solids.discard(key)
            else:
                continue
        if ct.is_in_vision(target):
            ...
            if building_id is not None:
                if (ct.get_entity_type(building_id) == EntityType.LAUNCHER
                    ...
                    continue
                # Something else stands here -- most often our own conveyor,
                # whose line to the Core runs straight through the ring. The
                # old code marked the site done, which silently left that arc
                # of the rim uncovered: a hole exactly where the enemy walks
                # in. Re-solve the cover without this tile so some other site
                # takes over its rim tiles.
                if key not in p.ring_blocked:
                    p.ring_blocked.add(key)
                    p.launcher_ring_targets = _launcher_ring_targets(
                        p, enemy_core, p.ring_blocked)
                    return False
                p.launcher_ring_done.add(key)
...
def _core_danger_zone(p):
    """Tiles an enemy turret can actually shoot the Core from.

    A Gunner fires a ray down one of the eight compass directions, so it only
    threatens the Core from a tile aligned with a Core tile: three out
    cardinally (9 <= 13) and two out diagonally (8 <= 13, where three would be
    18). Against a 2x2 Core that is a plus with three-tile arms and a 2x2 block
    at each corner -- appreciably smaller than the disc a plain radius test
    gives, which is why the earlier ring sat too far out and cost too many
    Launchers.
    """
    danger = set()
    for tile in p.foot:
        for direction in D8:
            ...
            for step in range(1, 4):
                ...
                if _distance_sq(spot, tile) > CORE_THREAT_RADIUS_SQ:
                    break
                danger.add(spot)
    return danger
...
def _core_threat_boundary(p):
    """The rim an enemy has to step onto before it can build in the danger zone.

    A Builder builds onto a cardinally adjacent tile, so the tiles it must be
    denied are the danger zone plus its cardinal neighbours. Only the rim needs
    Launchers -- the inside cannot be reached without crossing it.

    Rim means reachable rim. A tile whose outward neighbours are all map edge
    or wall is not a way in, and covering it is a Launcher spent on a direction
    nothing can arrive from. On a Core tucked into a corner that is most of the
    ring. Which tiles those are is settled by flooding the map inward from the
    border, not by measuring distance from the Core.
    """
    danger = _core_danger_zone(p)
    standing = set(danger)
    for tile in danger:
        for dx, dy in D4_DELTAS:
            standing.add((tile[0] + dx, tile[1] + dy))
    standing = {spot for spot in standing if _inside(p, spot)}

    # Everything an enemy can stand on without first entering the standing zone.
    frontier = deque()
    outside = set()
    border = [(x, y) for x in range(p.w) for y in (0, p.h - 1)]
    border += [(x, y) for y in range(p.h) for x in (0, p.w - 1)]
    for spot in border:
        if spot not in standing and spot not in p.walls and spot not in outside:
            outside.add(spot)
    ...
    while frontier:
        ...
        for dx, dy in D4_DELTAS:
            spot = (x + dx, y + dy)
            if (not _inside(p, spot) or spot in outside
                    or spot in standing or spot in p.walls):
                continue
            outside.add(spot)
            frontier.append(spot)

    perimeter = {spot for spot in standing
                 if any((spot[0] + dx, spot[1] + dy) in outside
                        for dx, dy in D4_DELTAS)}
    return perimeter, standing
...
def _shell_cover_targets(p, enemy_core, exclude=()):
    ...
    """
    shell, _standing = _core_threat_boundary(p)
    # A Launcher is solid. One dropped beside a deposit walls the Harvester
    # off, so ore tiles and the tiles that reach them are last resorts.
    ore_access = {(ore[0] + dx, ore[1] + dy)
                  for ore in p.ores for dx, dy in D4_DELTAS}
    buildable = {spot for spot in shell
                 if spot not in p.walls and spot not in p.ores
                 and spot not in p.foot and spot not in exclude}
    preferred = buildable - ore_access
    covers = {}
    ...
    while uncovered:
        ...
        for site, covered in covers.items():
            ...
            if gain == 0:
                continue
            key = (site not in preferred, -gain,
...
def _compass_ring_targets(p, enemy_core, exclude=()):
...
def _launcher_ring_targets(p, enemy_core, exclude=()):
    """The ring, in whichever geometry RING_COVER_SHELL selects."""
    if RING_COVER_SHELL:
        return _shell_cover_targets(p, enemy_core, exclude)
    return _compass_ring_targets(p, enemy_core, exclude)
    ...
    for direction in D8:
        ...
        for enemy_id in enemies:
            ...
            if (facing is not None
                ...
                candidates.append((
                    -TARGET_VALUE.get(ct.get_entity_type(enemy_id), 0),
                    position.distance_squared(target),
...
def _engage_with_turret(p, ct):
    ...
    """
    if (p.field_gunners_built >= MAX_FIELD_GUNNERS
...
def _hop_via_known_launcher(p, ct, target):
    """Walk to a Launcher we have seen before and ride it toward the target.

    Yes -- the Builder keeps its own map: p.seen, p.terrain, p.walls, p.ores
    and now p.friendly_launchers, which survives the Launcher leaving vision.
    Without that memory an attacker only ever used a Launcher it could see at
    that moment, so the ring it walked past ten tiles ago never carried it
    anywhere. A throw covers up to sqrt(26) tiles in one action; walking that
    is five turns it does not have.

    The trip is only worth it if the whole thing beats walking, and the whole
    thing includes waiting. A Launcher cannot read a request until the round
    after it is written, so the passenger gives up the round it announces in
    and the round it stands there waiting -- LAUNCH_OVERHEAD_ROUNDS. Comparing
    only "is the Launcher nearer the target than I am" ignored that entirely
    and sent Builders on detours that lost time overall.
    """
    here = tuple(ct.get_position())
    goal = tuple(target)
    direct = _cached_path(p, ct, here, goal, False)
    walking = len(direct) - 1 if direct else _cardinal_distance(here, goal)
    options = []
    for launcher in p.friendly_launchers:
        walk = _cardinal_distance(here, launcher)
        if walk <= 1 or walk > LAUNCHER_HOP_WALK:
            continue
        # Where the throw leaves us, at best: it advances up to LAUNCH_REACH
        # tiles along the way.
        # _cardinal_distance is Manhattan, which is the real walking cost.
        after = max(0, _cardinal_distance(launcher, goal) - LAUNCH_REACH)
        cost = walk + LAUNCH_OVERHEAD_ROUNDS + after
        if cost >= walking:
            continue
        options.append((cost, walk, launcher))
    if not options:
        return False
    *_, launcher = min(options)
    return _step(p, ct, Position(*launcher), False, allow_launcher=False)
...
def _opening_ferry(p, ct, enemy_core):
    ...
    launchers = _visible_friendly_launchers(ct)
    adjacent = _adjacent_visible_launcher(ct, target, launchers)
    if adjacent is None and _hop_via_known_launcher(p, ct, target):
        return True
    if adjacent is not None:
        p.awaiting_launch = LAUNCH_REQUEST_ROUNDS
        p.launch_origin = here
