...
# A fourth bit rather than a fourth doctrine: it rides in the same store word
# (pack_core shifts the doctrine into the high bits of a full 32-bit int) so
# every unit agrees on it, and it composes with all three openings.
#
# Measured. A second economy Builder is not generally good or generally bad --
# it depends on how far the ore is. On maps/orerich, where the nearest deposit
# sits a median of 2 tiles from a Core, it is -1.2 sd (`saga`). On the fifteen
# maps that reliably produce 200-400 round games, where the nearest deposit is
# a median of 4 tiles out and this bot builds 1.23 Harvesters a game, it is
# **+0.083 (+2.2 sd) over 240 strictly paired cells against eight opponents**,
# and McNemar on the discordant pairs gives z = +2.89 (34 cells won that the
# one-miner build loses, against 14 the other way). Harvesters go 1.23 -> 1.77.
#
# The Core can see this at round 0: its vision is r^2 = 36 and the deciding
# distances are 2 against 4, so this is a runtime reading of the board, not map
# knowledge.
FAR_ORE = 4
# Chebyshev distance from the Core to the nearest ore it can see. Above this a
# single miner spends the game walking and belting one deposit; the split in
# the data is 2 against 4, so the cut goes between them.
FAR_ORE_DISTANCE = 3

NAMES = {RUSH: "rush", FORTIFY: "fortify", BLITZ: "blitz",
         RUSH | 4: "rush+farore", FORTIFY | 4: "fortify+farore",
         BLITZ | 4: "blitz+farore"}
...
def _far_ore(ct) -> bool:
    """True when the nearest ore the Core can see is far enough to need a second
    miner. Never raises: an unreadable board falls back to the one-miner plan."""
    try:
        ...
        for tile in ct.get_nearby_tiles():
            ...
            if ct.get_tile_building_id(tile) is not None:
                continue
            distance = max(abs(tile.x - core.x), abs(tile.y - core.y))
            if best is None or distance < best:
                best = distance
        return best is not None and best > FAR_ORE_DISTANCE
    except Exception:  # noqa: BLE001 - a bad read must not cost the game
        return False
...
def _classify(ct) -> int:
    ...
    core = ct.get_position()
    far = FAR_ORE if _far_ore(ct) else 0
    if core_distance(ct) <= BLITZ_MAX_DISTANCE:
        # A Core six tiles away is decided before a second miner returns
        # anything, so BLITZ never takes the extra body.
        return BLITZ
...
def economy_builders(doctrine: int) -> int:
    extra = 1 if doctrine & FAR_ORE else 0
    doctrine &= ~FAR_ORE
    return _ROLES.get(doctrine, _ROLES[RUSH])[0] + extra
