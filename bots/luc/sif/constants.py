...
LAUNCHER_BUILDERS = 1
# sif: 2 -> 3, the ring Builder still spawns last behind two miners and one
# attacker.
...
# hodr: 1 -> 2. The '2, 3 and 4 all score like 1' measurement was made under
# Gunner-first delivery, where the attacker's budget and lifetime went to
# Gunners; Sentinel-first is a different regime and two is what breaks the
# 8 HP/round two-mender equilibrium.
SIEGE_SENTINEL_TARGET = 2
# Sentinel-first begins here; before it the attacker keeps the Gunner-tempo
# opening that the whole lineage's rush pressure is built on.
SIEGE_SENTINEL_FROM_ROUND = 60

# --- Late attack wave --------------------------------------------------------
# SIEGE_SENTINEL_TARGET's own measurement said the siege ceiling is delivery,
# not permission: one attacker cannot survive or stay solvent long enough to
# seat a second Sentinel. sporks (ladder #1, 2030) resolves it with bodies --
# 10+ Builders and a 12-21 Harvester economy, quiet until round ~200, then a
# wave that takes a full-HP Core down in ~70 rounds (watched vs Erebus and
# Pantheon, 2026-08-08). This is that wave on our chassis: once the opening has
# clearly not ended the game and the bank holds genuine surplus, the Core
# spawns extra attackers, each of which seats its own siege Sentinel from its
# own line (SENTINEL_SPREAD_LINES keeps them off shared rays).
#
# The wave window rides SLOT_OWN_CORE (see utils.pack_core): the Core
# announces the window one round before each spawn, because a store write is
# invisible until the next round and the new Builder must read its role on its
# first turn.
WAVE_ENABLED = True
WAVE_ROUND = 170
# Genuine surplus only: held beyond the Builder itself so the wave never taxes
# ammunition, mending, or the expansion economy that is funding it.
WAVE_BANK_RESERVE = 120
WAVE_MAX_ATTACKERS = 3
