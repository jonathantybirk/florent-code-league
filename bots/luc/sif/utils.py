...
# The own-Core slot also carries the late attack wave: which spawn indices are
# attackers rather than forced late miners (see LATE_BUILDERS_MINE). A window
# [start, start + count) rather than a threshold, so miners spawned after the
# wave closes go back to mining. Layout, low to high: position (10), doctrine
# (2), wave start (6), wave count (3).
DOCTRINE_MASK = 3
WAVE_START_SHIFT = POSITION_BITS + 2
WAVE_START_MASK = (1 << 6) - 1
WAVE_COUNT_SHIFT = WAVE_START_SHIFT + 6
WAVE_COUNT_MASK = (1 << 3) - 1


def pack_core(pos: Position | tuple[int, int], doctrine: int,
              wave_start: int = 0, wave_count: int = 0) -> int:
    """Core position plus the doctrine every unit has to agree on."""
    return (pack_pos(pos) | (doctrine << POSITION_BITS)
            | (min(wave_start, WAVE_START_MASK) << WAVE_START_SHIFT)
            | (min(wave_count, WAVE_COUNT_MASK) << WAVE_COUNT_SHIFT))


def unpack_core(value: int) -> tuple[tuple[int, int] | None, int]:
    return (unpack_pos(value & POSITION_MASK),
            (value >> POSITION_BITS) & DOCTRINE_MASK)


def unpack_wave(value: int) -> tuple[int, int]:
    """(start, count) of the attack-wave spawn-index window."""
    return ((value >> WAVE_START_SHIFT) & WAVE_START_MASK,
            (value >> WAVE_COUNT_SHIFT) & WAVE_COUNT_MASK)
