...
AMMO_TARGET = 60
# Past this round the bank itself is a win condition. Games that reach the
# 1000-round limit having collected nothing on either side are decided on
# titanium *stored*, and we lose 176 of 199 of them with a median bank of 16
# against 68 -- we spend the opening 500 on turrets and ammunition while the
# opponent sits on it. Ammunition is the largest discretionary sink, so past
# this round it stops being topped up.
LATE_BANK_ROUND = 880
...
def _keep_ammunition(ct, extra_reserve: int = 0) -> None:
    ...
    try:
        ...
        if ct.get_current_round() >= LATE_BANK_ROUND:
            return
        if held >= AMMO_TARGET:
            return
