...
# Lucas's alternative doctrine, 2026-08-04: "a lot of builder bots just
# constantly repairing our base while we try to find all mineral and route
# them back to our base while destroying enemy supply lines."
#
# The dedicated attacker becomes a second miner. Nothing else has to change,
# because the rest of that description is already built: late expansion fields
# up to seven Builders, a miner with no deposit left to claim falls through to
# the harass phase and goes after their belt, and the guard Builder is
# untouched. So this is the same bot with its one attacker reassigned, which
# makes it a clean arm rather than a rewrite.
_ROLES = {
    RUSH: (2, 0),
    FORTIFY: (2, 0),
