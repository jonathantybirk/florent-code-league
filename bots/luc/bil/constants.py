...
# A team-wide ceiling on turret spending, priced in cost scale.
#
# Every turret cap in this bot is per-Builder (`p.home_gunners_built`,
# `p.field_gunners_built`, `p.attack_gunners_built`), so the *team* total scales
# with the Builder count. Internally that is invisible -- our own bots do not
# press us, so we build 3.3 Gunners a game. Live it is not: decoded ladder
# replays show us building **22.7 Gunners a game** against Pivot's 11.6, and
# Pivot wins. At +20 scale each that is roughly 450 points of permanent tax,
# which is the thing that prices out the economy those same replays say we
# lack.
#
# The comms store is full, so a shared headcount is impossible -- but
# `get_scale_percent` is global, free and exact, and it measures the thing we
# actually care about. Past this ceiling a turret is refused whatever its
# per-Builder budget says.
TURRET_SCALE_CEILING = 350
