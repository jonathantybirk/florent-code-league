...
from constants import (TURRET_SCALE_CEILING,
...
def _build_blocker_gunner(p, ct, target):
    ...
    *_, position, facing = min(candidates)
    _gated_gunner(ct, position, facing)
...
def _gated_gunner(ct, *a, **kw):
    if not _turret_budget_left(ct):
        return None
    return ct.build_gunner(*a, **kw)


def _gated_sentinel(ct, *a, **kw):
    if not _turret_budget_left(ct):
        return None
    return ct.build_sentinel(*a, **kw)


def _turret_budget_left(ct):
    """Team-wide turret ceiling -- see TURRET_SCALE_CEILING."""
    return ct.get_scale_percent() < TURRET_SCALE_CEILING

def _build_turret(ct, kind, position, facing):
    if kind is EntityType.SENTINEL:
        _gated_sentinel(ct, position, facing)
    else:
        _gated_gunner(ct, position, facing)
