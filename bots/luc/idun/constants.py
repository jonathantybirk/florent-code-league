...
# Two claim slots is a hard cap of two simultaneously claimed deposits, and
# therefore a hard cap of about two Harvesters. Decoded from live ladder games,
# that is exactly the shape of our economy: Harvesters alive at round 50 -> 400
# run 1.6 -> 1.5 against Coreflood and 3.2 -> 0.3 against Besvikomat, while the
# same opponents go 1.6 -> 6.2 and 2.2 -> 6.3. Their economy compounds and ours
# is pinned flat all game. _pick's own comment already noted "an economy that
# never grows" and blamed claim staleness; staleness was real, but two slots is
# a ceiling no freshness policy can lift.
#
# Slots 6 and 7 come out of the launch-request range, which keeps four. That is
# the trade freyja made, and it was never measured clean because freyja carries
# RING_AFTER_ECONOMY, independently worth -8.3 points.
CLAIM_SLOTS = (1, 8, 6, 7)
LAUNCH_REQUEST_SLOTS = range(2, 6)
