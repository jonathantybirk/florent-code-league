...
LAUNCHER_BUILDERS = 1
# 0=miner, 1=miner, 2=attacker, 3=ring -- a miner added, not reallocated.
LAUNCHER_BUILDER_INDEX = 3
...
# sporks, rank 1 at 2115, seats its first Sentinel at rounds 6, 7, 10, 17 and
# 36 across a 5-0 sweep, with one Gunner and that one arriving after round 100.
# A 40-ammo gate cannot produce that; this is the ammunition a Sentinel needs to
# fire, not a comfortable reserve.
MIN_AMMO_FOR_SENTINEL = 12
...
# Order deposit candidates by distance to the Core and the belt we already own,
# rather than to the Builder. See _pick: the list is truncated, so the ordering
# decides what is considered at all.
# Try the siege Sentinel before the siege Gunner, not after it.
#
# The order was Gunner site, then Launcher-breaker, then Sentinel only if no
# Gunner site survived -- so the Sentinel fired 0.02 times a game. Decoding
# sporks (rank 1, 2115) sweeping not adgato 5-0, its Sentinels sit **2 to 6
# tiles from the enemy Core**, seated between rounds 6 and 36: a forward siege
# battery, not home defence. A Sentinel's line is never blocked and reaches
# r^2=32, which is what makes that seat work where a Gunner's 13 does not.
SIEGE_SENTINEL_FIRST_TRY = True
ANCHOR_ORE_ON_NETWORK = True
...
# ...but "this close" was closer than a Sentinel can shoot, which is the whole
# defect. A Sentinel reaches r^2=32 and a siege Sentinel is seated at the far
# end of that reach, where the Core's defenders cannot answer it -- so the
# mender standing on the Core sits at about 32, outside this radius, and rule 1
# is skipped in favour of shooting the Core. That is the "paying to lose
# slowly" the note above warns against, in the one place the radius could not
# reach.
#
# Live replays recorded two of five games lost to Pivot ending with our own
# Core untouched while we dealt 2828 and 4123 damage to theirs without killing
# a 500-HP Core, both lost on the round-1000 tiebreak. And 300 local games say
# the games we lose are the ones where we deal MORE Core damage than the ones
# we win, their Core ending at 322 of 500 -- about 1230 HP healed back, one
# mender working uninterrupted.
#
# Measured on snotra_h this was inert, because that base seats 0.32 Sentinels a
# game and three games in four have no Sentinel for the radius to govern. This
# base seats 0.83, so the same fix has more than twice the opportunity to fire.
BUILDER_PRIORITY_RADIUS_SQ = 32
