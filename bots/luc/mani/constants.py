...
# A threshold the bank never reaches is a mechanic that never runs.
#
# The Core replaces a lost Builder when the bank is large, on the reasoning that
# a healthy team spends income as it arrives, so a big idle bank means the
# workforce cannot keep up. Instrumented at the check itself over four games:
# **bank median 175, max 226, and 0 of 30 checks reached 260.** Our lineage has
# therefore never replaced a Builder it lost.
#
# `steward@e55aab5` uses **110** and passes 23 of those same 30 checks -- and it
# has the best live record of any build we own: **0.581 over 1,425 ladder games**
# against 30 opponents with a median rank of 19, ahead of every
# steward_hardened_reinforced variant and of snotra_h. Its commit message is
# explicit about what the threshold is for: "a team that loses two of three
# never replaces them and mines into a bank nothing is left to spend".
