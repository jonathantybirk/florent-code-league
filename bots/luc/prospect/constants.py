...
# anywhere including unclaimed ground, now set per doctrine in doctrine.py.
#
# It was shipped at 0, i.e. off, against expectation: measured over the 21
# official maps in both orders it cost 3 games out of 42 against mistral_fast
# at a cap of 3, and still 1 at a cap of 1, so the cap was not the problem --
# answering a roaming enemy with a building trades a mobile Builder's turn plus
# a permanent +10% for a turret the enemy simply walks away from. That is a
# statement about open ground, which is where 14 of the 21 maps are fought, and
# averaging it over the whole pool hid the case where the enemy cannot walk
# away. Under FORTIFY it is back on.
