...
GUARD_TURRET_SENTINEL = False
DEFEND_TURRET_SENTINEL = False
...
# than never laying.
LOCK_WAIT_LIMIT = 40
# Rounds the attacker will stand beside a legal siege seat it cannot yet
# afford before giving up and doing something else. Falling through instantly
# spends the titanium on a 20 Ti Gunner that the 30 Ti Sentinel was a few
# rounds away from, and 2.3.4 priced the Gunner worse on every axis but that
# 10 Ti. Waiting forever is what parked Builder id=6 for 33 of 44 rounds.
SIEGE_SAVE_ROUNDS = 150

# Gunners the attacker will buy at the enemy Core once the siege Sentinel is
# unaffordable or unseatable. Five was set when a Gunner cost 10 Ti and +10%
# scale; 2.3.4 doubled both and cut its damage, so each one is now the same
# +20% permanent tax as the Sentinel it is standing in for, levied on the
# economy that wins the round-1000 tiebreak.
ATTACK_GUNNERS = 5
