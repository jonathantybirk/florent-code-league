...
# ...but "this close" was closer than a Sentinel can shoot, and that is the
# whole defect. A Sentinel reaches r^2=32, and a siege Sentinel is deliberately
# seated at the far end of that reach, where the Core's own defenders cannot
# answer it. The mender standing on the Core is therefore at ~32 -- outside
# this radius -- so rule 1 is skipped and the Sentinel shoots the Core instead:
# precisely the "paying to lose slowly" the comment above warns against, in the
# one situation the radius was too small to cover.
#
# Two independent measurements say the same thing. From live replays, two of
# five games lost to Pivot ended with our own Core untouched while we dealt
# 2828 and 4123 damage to theirs without killing a 500-HP Core -- both running
# the full 1000 rounds, both lost on the titanium tiebreak. And over 300 local
# games against spar_sentinel, the games we *lose* are the ones where we deal
# MORE Core damage than the ones we win (1408 against 1290), their Core still
# ending at 322 of 500: about 1230 HP healed back, ~4.3 a round, which is one
# mender working without interruption.
#
# Matching the radius to the reach costs nothing when no Builder is on the
# line, because rule 1 only fires on a target the Sentinel could already shoot.
