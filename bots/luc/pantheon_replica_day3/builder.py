...
def _run(p, ct):
    ...
    if not hasattr(p, "builder_index"):
        ...
        p.is_launcher_builder = (
            0 <= p.ring_slot < doctrine.launcher_builders(p.doctrine)
        )
        # Pantheon's roles go by throw order. Builder 0 raids too -- it puts
        # the pad up on round 1 and is the first thing thrown off it on round
        # 2, which is why the raid lands so early. Ragnarok's launcher Builder
        # stays home and mines instead, and that is two rounds and one raider
        # of difference.
        p.is_attacker = p.builder_index < PANTHEON_RAIDERS
