...
def _run(p, ct):
    ...
    # ...and it joins on *any* Core damage, not only on the death projection.
    #
    # Every loss to the two bots that hold this bot's floor is an early rush:
    # against prospect on generated maps the losses come at a median turn of
    # 108 and a lower quartile of 75, against vigil 142 and 94, while the wins
    # come later. Survive the first hundred rounds and the game is won -- we
    # already take 113 of 164 from prospect.
    #
    # CORE_DYING cannot serve that window: it is gated behind
    # PROJECT_GRACE_ROUND and needs PROJECT_WINDOW rounds of history, so it
    # cannot fire before round 40 by construction. Plain damage can. And the
    # cost of being wrong is one Builder-turn of 1 Ti, against the +20%
    # permanent scale that every turret answer costs -- which is exactly why
    # the old "-30pp refill catastrophe" does not apply: that was bodies bought
    # to build turrets, not an existing body switching to healing.
    core_position = Position(*p.core)
    core_id = (ct.get_tile_building_id(core_position)
               if ct.is_in_vision(core_position) else None)
    hurt = (core_id is not None
            and ct.get_hp(core_id) < ct.get_max_hp(core_id))
    if ((ct.read_store(SLOT_CORE_DAMAGED) & CORE_DYING_FLAG) or hurt) \
            and not p.is_attacker:
        _heal_core(p, ct)
        return
