...
def _run(p, ct):
    ...
    if p.is_launcher_builder:
        ...
        if raw_alarm & CORE_DYING_FLAG or alarm:
            # A 3 Ti barrier in a Gunner's lane absorbs its whole clock: 30 HP
            # against 7 damage a shot is five shots and twenty of their
            # ammunition, and it is rebuilt for 3 Ti while they pay 4 Ti a shot
            # to knock it down. Measured in the lab at a Core held to zero
            # damage through 201 rounds of sustained fire. Tried before healing
            # because it stops the damage rather than repairing it, and falls
            # through to healing when no lane is blockable -- a Sentinel's line
            # cannot be blocked at all.
            enemies = [e for e in ct.get_nearby_entities()
                       if ct.get_team(e) != ct.get_team()]
