...
# Re-plan through transient bodies when the strict route fails. Off restores
# the odin behaviour exactly, so this is the ablation switch for the change.
SOFT_BODY_FALLBACK = True
...
def _bfs_path(p, source, target, exact, avoid_launchers=True, extra_blocked=(),
              require_free_step=False):
    """Full cardinal route from source to a goal tile, or None.

    Bodies are searched twice. A Builder standing in the way is not terrain:
    it is gone in a round or two, while a wall is there for the match. The
    strict pass keeps the old behaviour and is still preferred, so a route
    around a body is taken whenever one exists. Only when the strict pass
    finds nothing do we re-plan treating bodies as passable, which on a
    one-corridor map is the difference between walking up to the obstruction
    -- by which time it has usually moved -- and declaring no route at all
    and paying for a breaker Gunner or an escape Launcher to solve a
    teammate standing on a tile.

    `require_free_step` is the movement caller's extra demand: a soft route is
    only worth taking if the very next tile is walkable *now*. Planning
    through a body several tiles ahead buys real progress; planning through
    one standing directly in front buys a turn of standing still, when the
    old recovery chain -- breaker Gunner, escape Launcher, local sidestep --
    would have done something. Route planning for conveyor lines does not set
    this, because a belt is laid over ground bodies have long since left.
    """
    for soft_bodies in (False, True):
        path = _bfs_path_once(p, source, target, exact, avoid_launchers,
                              extra_blocked, soft_bodies)
        if path is not None:
            if (soft_bodies and require_free_step and len(path) >= 2
                    and path[1] in p.bot_occupied):
                return None
            return path
        if not SOFT_BODY_FALLBACK:
            return None
    return None


def _bfs_path_once(p, source, target, exact, avoid_launchers, extra_blocked,
                   soft_bodies):
