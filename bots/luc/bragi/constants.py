...
# ...and that "extra reach buys nothing" is what FIELD_TURRET_SENTINEL tests,
# because five live replays against Big O (rank 11, we score 3/25) say the
# opposite. Big O builds exactly two Sentinels in every game and almost nothing
# else; we answer with Gunners every game. At r^2=32 against 13 its turret
# opens fire from outside the range our field Gunner can ever answer from, so
# the seat is not a duel our reach loses -- it is a duel our reach never joins.
# Two of those games ended with 72-76 ammunition unspent, which is what being
# out-ranged looks like in the economy: shots we had paid for and could not
# take.
#
# The home-guard rationale above stays true where it was measured; this is the
# field path, against a Sentinel opponent, which is not the case it was
# measured on.
