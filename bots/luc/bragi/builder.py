...
def _engage_with_turret(p, ct):
    ...
    # Same shape as the home-guard path in _defend_core: pick the kind, search
    # seats at that kind's reach, and fall back to a Gunner when a Sentinel
    # finds no seat. The fallback is the whole safety net -- a Sentinel that
    # was unaffordable or unseatable must never cost us the turret entirely.
    kind = _turret_kind(ct, FIELD_TURRET_SENTINEL, p)
