...
def _step(p, ct, target, exact, allow_launcher=True):
    ...
    if (allow_launcher and not launch_rejected and not p.launch_blocked
            ...
            and _build_escape_launcher(p, ct, target, routine=False)):
        return True
...
def _build_escape_launcher(p, ct, target, routine=True):
    ...
    # Cap the *routine* ferry only. Capping both paths is what made
    # warden_walk a Nash specialist instead of a stronger bot: measured per
    # opponent, a blanket cap gains +14pp on ragnarok_fair, +12pp on valkyrie
    # and +10pp on ragnarok -- the Launcher-heavy bots that overspend when we
    # do not -- and loses 24pp on vigil@60d5afa, 19pp on vanguard_oracle and
    # 12pp on casemate_oracle, which are the ones that wall us out. Against a
    # turtle the chain is not overspending, it is the only way through.
    #
    # The two callers already distinguish the cases. `_opening_ferry` chains to
    # cross open ground and is what runs up the bill; `_step` only reaches for a
    # Launcher after PATH_FAILURES_BEFORE_LAUNCHER rounds of failing to path,
    # which is the signal that something is actually blocking us.
    if routine and p.relay_launchers_built >= MAX_ROUTINE_RELAY_LAUNCHERS:
        _plan_failed(
            p, ct, "build escape launcher", target,
            f"routine relay cap reached ({MAX_ROUTINE_RELAY_LAUNCHERS})",
        )
        return False
...
def _opening_ferry(p, ct, enemy_core, sighted=False):
    ...
    return _build_escape_launcher(p, ct, target, routine=True)
...
def _build_basic_gunner(p, ct, enemy_core):
    ...
    distances = _distance_map(p, me)
    protected_lanes = _friendly_turret_lanes(ct)
    # A seat an enemy turret already points at is shot before it fires much.
