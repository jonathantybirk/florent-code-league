...
from constants import (REPLACEMENT_BANK_THRESHOLD,
                       ...
                       WAVE_BANK_RESERVE, WAVE_ENABLED, WAVE_MAX_ATTACKERS,
                       WAVE_ROUND)
...
def run(player: "Player", ct: Controller) -> None:
    ...
    if not hasattr(player, "wave_start"):
        player.wave_start = 0
        player.wave_announced = 0
        player.wave_spawned = 0
    ct.write_store(SLOT_OWN_CORE,
                   pack_core(ct.get_position(), player.doctrine,
                             player.wave_start, player.wave_announced))
    ...
    # The late attack wave (see WAVE_ROUND). Announce-then-spawn: the store
    # write naming this index an attacker is invisible until next round, and
    # the Builder reads its role on its first turn, so the window must be
    # published one round before the body exists. Between announce and spawn
    # every other spawn path stands down, or an expansion miner would take the
    # announced index and walk to the enemy Core.
    wave_pending = player.wave_announced > player.wave_spawned
    waving = (
        WAVE_ENABLED
        and ct.get_current_round() >= WAVE_ROUND
        and resources >= builder_cost + WAVE_BANK_RESERVE
        and player.wave_spawned < WAVE_MAX_ATTACKERS
        and ct.get_current_round() >= player.last_spawn_round
        + REPLACEMENT_COOLDOWN_ROUNDS
    )
    if waving and not wave_pending:
        # Claim the next spawn index for the wave; the spawn itself happens on
        # a later round, once the window is visible to the Builder it names.
        if player.wave_start == 0:
            player.wave_start = role
        player.wave_announced += 1
        return
    if wave_pending:
        if not waving:
            # The bank dried up between announce and spawn; the announced index
            # stays claimed (a miner on it would be misread as an attacker), so
            # spawn nothing until the wave can afford its own body again.
            return
    elif (role >= MAX_OPENING_BUILDERS and has_live_builder and not expanding
            and not reviving):
    ...
    if wave_pending:
        # Wave attackers spawn facing the enemy, like the opening's attackers.
        mining_role = -1
        mines = False
    elif role >= MAX_OPENING_BUILDERS and ore_count:
    ...
    if candidates:
        ...
        if wave_pending:
            player.wave_spawned += 1
            print(f"WAVE round={ct.get_current_round()} "
                  f"attacker={player.wave_spawned}/{WAVE_MAX_ATTACKERS} "
                  f"index={role} bank={resources}",
                  file=sys.stderr, flush=True)
        if reviving:
