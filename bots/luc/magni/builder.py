...
from utils import (THROW_OFFSETS, is_response, landing_index, pack_enemy,
                   ...
                   unpack_core, unpack_enemy, unpack_pos, unpack_response, unpack_wave,
                   writable_by_builder)
...
def _run(p, ct):
    ...
    if not hasattr(p, "builder_index"):
        ...
        wave_start, wave_count = unpack_wave(ct.read_store(SLOT_OWN_CORE))
        if wave_count and wave_start <= p.builder_index < wave_start + wave_count:
            # The Core named this spawn index a late-wave attacker (see
            # WAVE_ROUND): a body bought from surplus to deliver one more siege
            # Sentinel, which is the one thing LATE_BUILDERS_MINE must not
            # convert into belt.
            p.is_launcher_builder = False
            p.is_attacker = True
        elif LATE_BUILDERS_MINE and p.builder_index >= MAX_OPENING_BUILDERS:
            p.is_launcher_builder = False
            p.is_attacker = False
