"""BASELINE for gl_guard: identical bot with NO Launchers built.

Same Core reporter, same lone builder, so the Core-HP numbers are directly
comparable.  Original gl_guard docstring follows.
...
class Player:
    ...
    def _core(self, ct, r):
        ...
        if r >= REPORT and not self.done:
            self.done = True
            ct.resign(("NULL r%d corehp=%d minhp=%d minr=%d launchers=%d "
