...
class Player:
    ...
    def _builder(self, ct, r):
        ...
        for t in open_now:
            ...
            stands[t] = (outside, inside)
        # Corner and edge Cores (sprint, jackpot, string) have ring tiles whose ONLY stand tile is
        # another ring tile.  Those must be bricked FIRST and from the dead end inwards -- walk the
        # arc and brick behind you -- because bricking the ring near your entry cuts you off from
        # the rest of it.  Hence: no-outside-stand tiles first, farthest first.
        order = sorted(open_now, key=lambda t: (
            1 if stands[t][0] else 0,
            abs(t[0] - pos.x) + abs(t[1] - pos.y) if stands[t][0]
            else -(abs(t[0] - pos.x) + abs(t[1] - pos.y))))
        for tgt in order[:8]:
            outside, inside = stands[tgt]
