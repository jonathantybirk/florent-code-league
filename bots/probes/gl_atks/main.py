"""ADVERSARY gl_atks: 4 DEMOLITION builders -- shooting has PRIORITY over moving.

The worst case for any Launcher: every builder that ends a round orthogonally
adjacent to an enemy building fires at it for 2 damage / 2 Ti (G59) before it
considers moving. A Launcher has 30 HP, so this asks whether a Launcher can
survive contact at all.

Whenever a builder cannot take its pathfinding step it fires at any orthogonally
adjacent enemy building (2 damage for 2 Ti, G59). This is the test of whether a
caged prisoner can chew its own cell open: a barrier is 30 HP, so 15 shots.
See gl_atk1 for the pathfinder design.
