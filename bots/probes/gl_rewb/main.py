"""MIRRORED rewind launcher, for TEAM B, so gl_spy can be the reporting victim.

Identical policy to gl_rew but sited for the east Core (23,6): Launcher at
(14,6), so on lab/glchoke the one-tile gap at (13,6) is inside its pickup ring.
Run:

    python tools/runprobe.py gl_spy --map lab/glopen  --vs gl_rewb
    python tools/runprobe.py gl_spy --map lab/glchoke --vs gl_rewb
...
PLAN = (("go", 15, 6), ("launcher", 14, 6), ("go", 17, 8), ("go", 20, 10))
...
class Player:
    ...
    def _launcher(self, ct, r):
        ...
        if r >= REPORT and not self.done:
            ...
            msg = ("REWB r%d built=%d rew=%d ids=%d ringmax=%d missed=%d "
