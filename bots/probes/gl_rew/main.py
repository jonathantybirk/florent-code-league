"""LAUNCHER Q3/Q4/Q5: REWIND-ONLY launcher -- no barriers, no cell.

Arena: maps/lab/glopen.map26.  Run against each adversary:

    python tools/runprobe.py gl_rew --map lab/glopen --vs gl_atk1
    python tools/runprobe.py gl_rew --map lab/glopen --vs gl_atk4
    python tools/runprobe.py gl_rew --map lab/glopen --vs gl_atkr
...
class Player:
    ...
    def _panic(self, ct, r, myhp, corehp):
        ...
        ct.resign(("REW-DEAD r%d lhp=%d hpmin=%d hpminr=%d corehp=%d gob=%d "
    ...
    def _launcher(self, ct, r):
        ...
        if r >= REPORT and not self.done:
            ...
            msg = ("REW r%d built=%d rew=%d ids=%d ringmax=%d missed=%d "
