...
ECON_BACKSTOP = 120        # pivot regardless if we have never even seen their Core by here
    ...
    def _rush_is_dead(self, ct):
        ...
        if seen:
            ...
            if self.foe_min is None or now < self.foe_min:
                self.foe_min = now
            elif False and now > self.foe_min + HEAL_EVIDENCE and now > NEARLY_DEAD:
                ...
                # still healthy.
                why = "healing"
        if False and self.converted > KILL_AMMO * SPEND_MULTIPLE:
