"""SABOTAGE Q4a -- what a BUILDER pays to shoot down one 30 HP BARRIER, on 2.3.6.
...
class Player:
    ...
    def _run(self, ct):
        ...
        if self.st == 3:
            ...
            bid = ct.get_tile_building_id(TGT)
            if bid is None:
                self.costs.append(ct.get_barrier_cost())
        ...
        if self.st == 4:
            ...
            for i in range(1, len(self.firerounds)):
                ...
                if g not in gaps:
                    gaps.append(g)
            ct.resign(
                "SBBARB build=%s hpladder=%s fires=%d r%s->%s gaps=%s "
