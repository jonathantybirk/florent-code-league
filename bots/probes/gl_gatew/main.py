"""THE GATE + a HEALING WARDEN: gl_gate with the builder parked at (10,5).

A caged builder shoots its cell wall for 2 damage a round (G59); heal restores 4
HP for 1 Ti (G60), so a warden orthogonally adjacent to the Launcher and to the
barrier at (10,4) out-repairs two attackers at half the titanium. This measures
whether the gate survives an opponent whose builders shoot buildings.
...
class Player:
    ...
    def _warden(self, ct):
        ...
        for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1)):
            ...
            if bid is None or ct.get_team(bid) != ct.get_team():
                continue
            hp = ct.get_hp(bid)
            if hp < ct.get_max_hp(bid) and hp < bhp and ct.can_heal(q):
                bhp = hp
                best = q
        if best is not None:
            ct.heal(best)
    ...
    def _panic(self, ct, r, myhp, corehp):
        ...
        ct.resign(("GATEW-DEAD r%d lhp=%d hpmin=%d hpminr=%d corehp=%d gob=%d "
    ...
    def _launcher(self, ct, r):
        ...
        if r >= REPORT and not self.done:
            ...
            ct.resign(("GATEW r%d built=%d penready=%d gob=%d distinct=%d esc=%d "
