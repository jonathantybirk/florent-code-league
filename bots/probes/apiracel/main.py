"""SILENT TWIN of probe `apirace` -- identical behaviour, never resigns.

Only one resign_message survives a game and resigning ends it, so the race needs one
reporting side and one quiet side.  This twin spawns its builder on round 10, so ITS builder has the HIGHER entity id -- the id-order discriminator.
