...
FLEET_SEAMS = 3            # Harvesters the WHOLE fleet builds, not per Builder.
                           # hildr fields 10-11 Builders and only 2-3 Harvesters: the rest MEND.
                           # Ours all went mining, so none was ever beside the Core to heal it and
                           # we cast zero heals a game against hildr's 190-250. Mending returns
                           # 4 HP per titanium against a Sentinel shot's 1.8, so the miners are the
                           # minority of a working economy, not the whole of it.
HARVESTER_TARGET = 1       # seams per Builder, still capped by FLEET_SEAMS above
...
SLOT_SEAMS = 5             # Harvesters the fleet has built, so miners can stand down
...
class Player:
    ...
    def _econ(self, ct, here):
        ...
        fleet = 0
        try:
            fleet = ct.read_store(SLOT_SEAMS)
        except Exception:
            fleet = 0
        if self.mined >= HARVESTER_TARGET or fleet >= FLEET_SEAMS:
                ...
                if abs(ore[0] - here.x) + abs(ore[1] - here.y) == 1:
                    try:
                        if ct.can_build_harvester(Position(ore[0], ore[1])):
                            ...
                            self.mined += 1
                            try:
                                ct.write_store(SLOT_SEAMS, fleet + 1)
                            except Exception:
                                pass
                            self.chain = None
                            return
