...
class Player:
    def __init__(self):
        ...
        self.bodies = set()         # enemy Builder Bots THIS ROUND -- transient, never cached
        self.built = 0
        self.width = 0
        self.height = 0
    ...
    def _see_bodies(self, ct):
        """Enemy Builder Bots block a tile just as hard as a wall, and we never looked.

        `blocked` and `occupied` are both filled from get_tile_building_id, and A BUILDER BOT IS
        NOT A BUILDING. So a parked enemy body passed every occupancy test: the router planned
        straight through it and can_move refused, and `_free_spot` called its tile buildable and
        can_build_sentinel refused. Measured in this repo's own exploit notes against a 60-line bot
        that parks six Builders and never moves again -- zero turrets in 41 of 42 games, 47,695
        refused builds. Our own helheim stall is the same thing: idle from round 11 to 27 with an
        enemy Builder sitting on (5,6).

        Rebuilt every round rather than cached, because unlike a building a body walks away.
        """
        self.bodies = set()
        try:
            mine = ct.get_team()
    ...
    def _passable(self, key):
        if key in self.walls or key in self.blocked or key in self.bodies:
            return False
    ...
    def _free_spot(self, key):
        ...
        return (key in self.spots and key not in self.walls
                and key not in self.occupied and key not in self.bodies
