"""Starter bot with the GCS as its communication layer (verification build).

Identical play to bots/starter, but every unit runs the Global Communication
...
_EIGHT = [Direction.NORTH, Direction.NORTHEAST, Direction.EAST, Direction.SOUTHEAST,
...
class Player:
    ...
    def _run_core(self, ct: Controller) -> None:
        ...
        if self.num_spawned >= MAX_BUILDERS:
            return
        # GCS: never spawn while a resync/onboarding window is open, and only
        # when a slot can be handed out in the spawn round
        reg = self.gcs.registry
        r = ct.get_current_round()
        if reg.in_onboard_window(r) or reg.is_resync_round(r):
            return
    ...
    def _try_build_gunner(self, ct: Controller) -> bool:
        ...
        for d in Direction:
            ...
            if ct.can_build_gunner(build_pos, facing):
                ...
                # GCS: tell everyone about the gunner and grant it a slot
                slot = self.gcs.registry.pick_turret_slot(spread=self.gcs.slot or 0)
                if slot is not None:
                    self.gcs.grant_turret(build_pos.x, build_pos.y, "gunner",
                                          _EIGHT.index(facing), slot)
                return True
        return False
