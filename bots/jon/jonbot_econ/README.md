...
Jonbot-family challenger based on `brynhildr@533601b`. It adds one store-free,
precomputed opening miner on 27 proven terrain/start pairs and otherwise runs
the parent unchanged. The original attacker begins on round 1 where the opening
is active. The vendored `openingstrat/` package keeps ladderfarm exports and
platform submissions self-contained.

Local evidence:

- Against its `533601b` parent: 27-0 at seed 1 on retained starts.
- Validation on seeds 2-4: 79-2; both losses were tied Pinch coinflips.
- Against the first queued v110-based econ fork over all 53 maps and both seats:
  59-47.
- The inherited 37-start gate was tested first and scored 27-10; all ten losing
  starts were removed before validation.
- All matches used the server 10 ms TLE.

This name signals the Jonbot family because it is the strongest local candidate,
not because it is already entitled to online promotion. Ladderfarm's qualified
delta-Elo decision remains authoritative.
