...
def sense(p, ct):
    ...
    if p.core and not p.atlas_checked:
        p.atlas_checked = True
        p.atlas = identify_visible(ct, p.core)
        if p.atlas is not None:
            tiles = {(x, y) for y in range(p.h) for x in range(p.w)}
            p.seen.update(tiles)
            p.terrain.update({tile: Environment.EMPTY for tile in tiles})
            ...
            p.enemy_core = p.atlas.enemy_core
            ct.write_store(SLOT_ENEMY_CORE, pack_pos(p.enemy_core))
