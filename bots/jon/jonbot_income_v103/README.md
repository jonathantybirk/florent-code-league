...
Timing-preserving mining-movement fork of the live ladderfarm flagship,
`brynhildr@84d92d3` (submission v103). Core economy authorization, spawning,
combat, defence, and every decision about *when* to build are unchanged.

The only behavioral change is how an authorized miner approaches and vacates
its next conveyor or Harvester tile: it stages inward on the completed chain
instead of taking an arbitrary side-step. The seven map/start combinations
that regressed in the earlier paired panel retain exact parent movement.
