...
def _run(p, ct):
    if not hasattr(p, "builder_index"):
        ...
        p.home_launcher_built = False
    _sense(p, ct)
    _update_enemy_core_inference(p, ct)
    if p.core is None:
        return
    if not p.is_attacker and _build_picket(p, ct):
        return
...
def _distance_map(p, source):
    """Compute every reachable distance once for multi-goal scoring."""
    blocked = p.walls | p.foot | p.solids | (p.bot_occupied - {source})
    dist, queue = {source: 0}, deque([source])
    while queue:
        cur = queue.popleft()
...
def _build_picket(p, ct):
    """Buy one repelling Launcher only when an enemy Builder reaches home."""
    if p.home_launcher_built:
        return False
    ...
    threats = [unit for unit in ct.get_nearby_units()
               ...
               and ct.get_position(unit).distance_squared(core) <= 100]
    if not threats or ct.get_global_resources() < ct.get_launcher_cost() + 60:
        return False
    me = ct.get_position()
    candidates = []
    for dx, dy in D4_DELTAS:
        spot = Position(me.x + dx, me.y + dy)
        if not _inside(p, tuple(spot)) or tuple(spot) in p.foot:
            continue
        if ct.can_build_launcher(spot):
            nearest = min(spot.distance_squared(ct.get_position(unit))
                          for unit in threats)
            candidates.append((nearest, spot.x, spot.y, spot))
    if not candidates:
        # Get back into the home ring before trying to place the picket.
        if me.distance_squared(core) > 20:
            _step(p, ct, core, False)
            return True
        return False
    *_, spot = min(candidates)
    ct.build_launcher(spot)
    p.solids.add(tuple(spot))
    p.home_launcher_built = True
    ...
    for core_tile in sorted(core_tiles):
        for dx in range(-3, 4):
            for dy in range(-3, 4):
                ...
                distance = min((distances[goal] for goal in goals
                                if goal in distances), default=None)
                if distance is not None:
                    choices.append((distance, spot, facing.value, facing))
    if not choices:
        _explore(p, ct)
        return True
