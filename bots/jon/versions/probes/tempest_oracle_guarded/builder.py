...
def _sense(p, ct):
    ...
    if p.atlas is not None:
        ...
        if mismatch:
            # A generated map can share an official map's dimensions, Core
            # anchor, and opening view. Revoke privileged predictions at the
            # first later contradiction instead of retaining phantom walls.
            p.atlas = None
...
def _rush(p, ct):
    ...
    if packed == 0:
        _explore(p, ct)
        return
    if _ferry(p, ct, unpack_pos(packed)):
        return
...
def _ferry(p, ct, enemy_core):
    """Give the lead attacker one atlas-directed hop on long approaches."""
    if p.builder_index != ECONOMY_BUILDERS or p.hops >= 1:
        return False
    ...
    if ct.get_global_resources() < ct.get_launcher_cost() + 60:
        return False
    blocked = p.walls | p.solids | p.ores
    for dx, dy in D4_DELTAS:
        spot = here[0] + dx, here[1] + dy
        if not _inside(p, spot) or spot in blocked:
            continue
