        ...
        for dx in range(-3, 4):
            for dy in range(-3, 4):
                ...
                if distance is not None:
                    # At equal travel time, stand as far from the Core as the
                    # Gunner permits. This keeps the engineer outside an
                    # adjacent defensive Launcher's control radius.
                    choices.append((distance, -_distance_sq(spot, enemy_core),
                                    spot, facing.value, facing))
    if not choices:
        _explore(p, ct)
        return True
