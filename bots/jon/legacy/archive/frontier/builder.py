...
def _read_assignment(player: Player, ct: Controller) -> None:
    ...
    if 0 <= role < BUILDER_COUNT and assignment >> 3 == ct.get_id():
        ...
        if role in ATTACKER_ROLES:
            ...
            player.attack_candidate = ATTACKER_ROLES.index(role)
            player.ore_target = None
            player.target = None
...
def _update_coverage(player: Player, ct: Controller) -> None:
    """Publish coverage, deposit count, and rejected symmetry hypotheses."""
    ...
    value = (
        player.coverage_mask
        | (min(player.deposits_built, DEPOSIT_COUNT_MASK) << 25)
        | (player.rejected_symmetries << SYMMETRY_REJECT_SHIFT)
    )
...
def _team_deposit_count(ct: Controller) -> int:
    return sum(
        (ct.read_store(SLOT_COVERAGE_START + role) >> 25) & DEPOSIT_COUNT_MASK
        for role in range(BUILDER_COUNT)
    )
...
def _attack_scout(player: Player, ct: Controller) -> None:
    ...
    if player.attack_core is None:
        ...
        rejected = player.rejected_symmetries
        for role in range(BUILDER_COUNT):
            rejected |= (
                ct.read_store(SLOT_COVERAGE_START + role)
                >> SYMMETRY_REJECT_SHIFT
            )
        available = [
            index for index in range(len(candidates)) if not rejected & (1 << index)
        ]
        if not available:
            # A stale or mistaken rejection must not permanently idle scouts.
            available = list(range(len(candidates)))
        preferred = player.attack_candidate % len(candidates)
        candidate_index = min(
            available,
            key=lambda index: ((index - preferred) % len(candidates), index),
        )
        ...
        if ct.is_in_vision(candidate):
            ...
            if core_here:
                ...
                player.rejected_symmetries |= 1 << candidate_index
                remaining = [
                    index
                    for index in range(len(candidates))
                    if not (rejected | player.rejected_symmetries) & (1 << index)
                ]
                if remaining:
                    candidate_index = min(
                        remaining,
                        key=lambda index: (
                            (index - preferred) % len(candidates),
                            index,
                        ),
                    )
                    candidate = candidates[candidate_index]
    ...
    if existing is not None and ct.get_entity_type(existing) == EntityType.HARVESTER:
        ...
        player.route_seen.clear()
        return
    # Never let forward construction bankrupt an unfinished home economy.
    if (
        _team_deposit_count(ct) >= 1
        and ct.get_global_resources()
        >= ct.get_harvester_cost() + ATTACK_RESERVE_TITANIUM
        and ct.get_action_cooldown() == 0
        and ct.can_build_harvester(player.attack_ore)
    ):
...
def _relay_toward(player: Player, ct: Controller, target: Position) -> None:
    ...
    if (
        not player.relay_built
        and pos.distance_squared(target) > 32
        and ct.get_action_cooldown() == 0
        and ct.get_global_resources()
        >= ct.get_launcher_cost() + ATTACK_RESERVE_TITANIUM
    ):
