...
def run(player: Player, ct: Controller) -> None:
    ...
    # One forward scout is enough to resolve symmetry and report the enemy.
    # The second attacker remains a productive miner until the Core confirms
    # that four supply lines and a spending reserve exist.
    if (
        player.phase == "explore"
        and player.role in ATTACKER_ROLES
        and not player.attack_retired
        and ct.read_store(SLOT_STRATEGY) & 1
    ):
...
def _read_assignment(player: Player, ct: Controller) -> None:
    ...
    if 0 <= role < BUILDER_COUNT and assignment >> 3 == ct.get_id():
        ...
        if ATTACKER_ROLES and role == ATTACKER_ROLES[0]:
            player.phase = "attack_scout"
            player.ore_target = None
            player.target = None
...
def _should_defend(
    ...
    if player.role not in DEFENDER_ROLES:
        return False
    threat_until = ct.read_store(SLOT_STRATEGY) >> 1
    if threat_until >= ct.get_current_round():
        return True
...
def _attack_scout(player: Player, ct: Controller) -> None:
    ...
    if ct.get_position() != staging:
        ...
        _move_toward(player, ct, staging, allow_diagonal=True)
        return
    if not ct.read_store(SLOT_STRATEGY) & 1:
        return
