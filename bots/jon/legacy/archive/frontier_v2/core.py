    ...
    for spawn_pos in spawn_tiles:
        if ct.can_spawn(spawn_pos):
            ...
            if role in ATTACKER_ROLES:
                player.attacker_ids[ATTACKER_ROLES.index(role)] = builder_id
...
def _initial_target(ct: Controller, role: int) -> Position:
    ...
    pos = ct.get_position()
    if role == ATTACKER_ROLES[0]:
        return Position(ct.get_map_width() - 2 - pos.x, pos.y)
    if role == ATTACKER_ROLES[1]:
    ...
    economy_rank = role - sum(attacker < role for attacker in ATTACKER_ROLES)
    if economy_rank < len(ores):
        return ores[economy_rank]
    ...
    return centres[(economy_rank - len(ores)) % len(centres)]
