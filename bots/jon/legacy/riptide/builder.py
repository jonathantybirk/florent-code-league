...
def _picket(p, ct):
    """Post Launchers on the approach to throw raiders back where they came."""
    ...
    if ct.get_global_resources() < ct.get_launcher_cost() + 40:
        return False
    ring = _core_ring(p)
    spots = sorted(t for t in ring
                   if t not in p.solids and t not in p.conveyors
                   and t not in world.known_walls(p)
                   and t not in world.known_ores(p) and t not in p.fortified)
    spots.sort(key=lambda t: world.dist_sq(t, p.enemy_core))
    for spot in spots[:2]:
