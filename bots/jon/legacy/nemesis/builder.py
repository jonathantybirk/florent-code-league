...
def _init(p, ct):
    ...
    # The first few Builders carry the economy that wins the tiebreak; every
    # later one is a warden whose whole job is that no firing position exists.
    p.warden = p.ticket >= MINERS
...
def _economy(p, ct):
    ...
    if _repair(p, ct):
        return
    # Split the labour instead of the priority. Doing denial first starves the
    # economy and loses the tiebreak; doing it second lets the siege land and
    # loses outright. Miners only mine, wardens only deny, and both repair.
    if p.warden:
        ...
        _explore(p, ct)
        return
    if p.phase in ("idle", "explore") and _fortify(p, ct, ring_only=True):
        return
    if p.phase == "idle":
        _pick_job(p, ct)
    if p.phase == "line":
...
def _threatened_tiles(p):
    """Every tile from which a Gunner could draw a line on our Core.

    This is the whole of Vanguard's attack surface. It needs a free tile that
    (a) touches a producer -- ours or one it plants -- and (b) has a line to the
    Core. Condition (b) alone is cheap to enumerate and cheaper to brick, and a
    tile holding a Barrier cannot be built on at all.
    """
    threats = set()
    for tile in world.footprint(p.core):
        for delta in FACING8:
            for step in range(1, 4):
                spot = (tile[0] - delta[0] * step, tile[1] - delta[1] * step)
                if not world.inside(p, spot) or spot in p.foot:
                    break
                if world.dist_sq(spot, tile) > GUNNER_RANGE_SQ:
                    break
                if spot in world.known_walls(p):
                    break
                threats.add(spot)
                if spot in p.solids or spot in p.conveyors:
                    break            # a building already blocks the line here
    return {t for t in threats if world.cheb(t, p.core) <= DENIAL_RADIUS}


def _home_gunner(p, ct):
    """Post a fed Gunner over the approach.

    Vanguard removed its own defensive Gunners because nothing it fought could
    punish that. Its attack is Builders walking in one at a time with no ranged
    attack, so a single fed Gunner kills each arrival for 2 Ti a shot.
    """
...
def _fortify(p, ct, ring_only=False):
...
def _support(p, ct):
    """Nemesis never attacks: it denies, repairs, and out-mines."""
    p.phase = "explore"
    _explore(p, ct)
