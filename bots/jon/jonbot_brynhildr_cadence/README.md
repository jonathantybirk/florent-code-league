# jonbot_brynhildr_cadence

Exact fork of ladderfarm's expected-Elo flagship `brynhildr@e4c3dff` (v104).
It changes only the generic miner's movement between builds: after laying an
outward conveyor it stages through the completed inward network, producing a
build/move cadence without changing when the Core buys economy. Antler and
Holmgang A retain the parent movement where paired tests show exposed-trunk
regressions.

Against Spar Econ over 15 pool maps, both seats, and three seeds, this fork
scores 84-6; the exact parent scores 81-9. Longhouse B flips at every tested
seed with no loss flips. A 90-game direct mirror against v104 scores 48-42.
