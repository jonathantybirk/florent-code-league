...
def run(player: "Player", ct: Controller) -> None:
    ...
    # An EXACT live headcount, which this bot was built believing it could not
    # have. Both the heartbeat and the replacement bank exist to work around
    # "buffered store writes make a live headcount impossible to publish" --
    # true of the store, and irrelevant, because get_unit_count() returns "the
    # number of living units currently on this unit's team, including the
    # core" directly to the Core. It is never called anywhere in this bot.
    #
    # The cost is recorded in the comment below: the heartbeat proves only that
    # ONE Builder lives, so a team that loses two of three never replaces them
    # and mines into a bank nothing is left to spend -- down to one Builder by
    # round 45 on sweden, dead on round 103 holding 358 titanium.
    live_builders = max(0, ct.get_unit_count() - 1)
    ...
    if role >= MAX_OPENING_BUILDERS and not expanding:
        # Replace on the real shortfall rather than on a bank large enough to
        # imply one. The bank proxy fires late by construction: titanium has to
        # pile up first, which is exactly the interval where the workforce is
        # already too small to spend it.
        if live_builders >= MAX_OPENING_BUILDERS:
            return
        if (ct.get_current_round()
                < player.last_spawn_round + REPLACEMENT_COOLDOWN_ROUNDS):
            return
