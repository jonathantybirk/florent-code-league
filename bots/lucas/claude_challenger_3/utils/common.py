...
def ordered_ores(km: KnownMap, my_core: Position) -> list[Position]:
    ...
    A single multi-source BFS from the Core's perimeter (one call to
    _distance_field) gives every position's distance to the Core at once.
    Static walking distances are symmetric — _distance_field's diagonal
    corner-cut check requires the same pair of corner tiles regardless of
    travel direction (moving current->candidate needs the same two tiles
    passable as candidate->current) — so an ore's distance to the Core is
    just the minimum of this one field over the ore's own adjacency. The
    previous version ran a separate reverse BFS per ore (via path_distance,
    once per Core-perimeter start): O(ore count) full-map BFS passes for the
    exact same answer this computes with one. Measured cost on atoll: ~21ms
    (over the 10ms/round budget) down to ~2.8ms.
    """
    ...
    core_distances = _distance_field(
        km, core_perimeter(my_core), cardinal_only=False
    )

    def key(ore: Position) -> tuple[int, int, int]:
        distances = [
            distance
            for adjacent in adjacent_positions(km, ore)
            if (distance := core_distances.get(adjacent)) is not None
        ]
