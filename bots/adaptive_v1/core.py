...
def run(player: Player, ct: Controller) -> None:
    ...
    old_strategy = ct.read_store(SLOT_STRATEGY)
    attack_ready = bool(old_strategy & 1) or (
        completed_lines >= 4
        and ct.get_global_resources() >= ATTACK_RESERVE_TITANIUM
    )
    if any(
        ct.get_team(unit_id) != ct.get_team()
        for unit_id in ct.get_nearby_units()
    ):
        player.threat_until = min(1023, ct.get_current_round() + 60)
    ct.write_store(
        SLOT_STRATEGY,
        int(attack_ready) | (player.threat_until << 1),
    )
