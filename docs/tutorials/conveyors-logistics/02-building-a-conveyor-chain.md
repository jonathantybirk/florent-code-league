...
Source: https://game.code.florent.vc/tutorials/conveyors-logistics/02-building-a-conveyor-chain
...
> **Correction vs. the official docs.** The published page uses `pos.cardinal_direction_to(target)` in `_seek_and_harvest` below. **This method does not exist** in the installed `fcode` engine (confirmed by direct call at runtime — raises `AttributeError`). The code below uses `pos.direction_to(target)` with a rotate-left/right fallback instead.
