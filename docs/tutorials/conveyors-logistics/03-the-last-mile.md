...
Source: https://game.code.florent.vc/tutorials/conveyors-logistics/03-the-last-mile
...
> **Correction vs. the official docs.** `_seek_and_harvest` still uses `pos.cardinal_direction_to(target)` on the published page — **this method does not exist** in the installed `fcode` engine (confirmed by direct call, raises `AttributeError`). Corrected below to `pos.direction_to(target)` with a rotate-left/right fallback, same as the previous tutorial.
