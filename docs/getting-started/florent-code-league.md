...
1. **Write your bot.** Implement a `Player` class in Python. The engine calls your `run()` method once per turn for each unit, passing a `Controller` object that exposes the full game API.
2. **Test locally.** Use the `fcode` CLI to run matches on your machine and watch replays in the visualiser.
3. **Submit.** Upload your bot via `fcode submit`. It is automatically entered into the ladder and matched against other submitted bots.
4. **Climb the ladder.** Win matches, accumulate rating, and compete for the top spot on the leaderboard.
...
Matches are run as a best-of-five series. Each series result is reflected on the leaderboard. You can submit as many times as you like — each submission replaces the previous one on the ladder.

## Ready to begin?

Head to [Quick Start](quick-start.md) to install the CLI and run your first match in under five minutes.
