...
| G46 | **A thrown Builder Bot lands on zero cooldowns and can move OR act on the landing round** — at full HP, with no landing penalty. | `VERIFIED-WIN` | Landed and moved east the same round on one arena, landed and *built a Barrier* the same round on another. Caveat: entities act in ascending id order, so this only works when the Launcher was built **before** the bot was spawned. |
| G47 | **Launchers chain within a single round.** Two Launchers moved one bot **12 tiles in one round**, landing at full HP on zero cooldown. | `VERIFIED-WIN` | Launchers at (6,5) and (12,5) relayed a bot (5,5)→(11,5)→(17,5) on round 22. A builder otherwise moves 1 tile/round and cannot move and act in the same round. |
| G48 | **The throw ARCS.** Walls and Core footprints do not block it. Legal targets are exactly the Euclidean disc r²≤26 from the **Launcher**, minus non-bot-passable tiles. | `VERIFIED-WIN` | A solid 7-tile wall column between Launcher and landing zone did not block; a full ±6 scan matched `dx²+dy² ≤ 26` tile for tile. |
| G49 | **Launcher pickup is TEAM-BLIND — an adjacent ENEMY Builder Bot can be thrown.** | `VERIFIED-WIN` | 81 legal targets measured for an adjacent enemy builder. A displacement weapon nobody in the field appears to know exists. |
| **G50** | **A Builder Bot's action radius is r²≤2 — DIAGONAL BUILDS ARE LEGAL.** `docs/game-rules/game-rules-builder-bot.md` says Build/Attack/Heal/Destroy are orthogonally adjacent only. **Wrong.** | `VERIFIED-WIN` | Measured directly. Corroborated independently by the spec bundled inside the fcode wheel, which says "within its action radius" — Lucas flagged the same discrepancy in his `docs/engine-spec.md`. **Doc error #8, and a capability `bot/main.py` has never used**: diagonal builds enlarge every build-site set, shorten chains, and speed siege setup. |
| G51 | A building does not act on the round it is built, so a self-ferry cycle (build Launcher → be thrown) takes two rounds, not one. | `VERIFIED-WIN` | Measured on a purpose-built arena. |
...
| ~~G41~~ | *resolved — see G46–G49 below* | — |
