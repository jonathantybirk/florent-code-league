...
|---|---|---|---|
| G01 | `titanium_collected` counts **only stacks that land on a Core footprint tile**. Passive income contributes 0. | `VERIFIED-WIN` | Idle bot: 3000 Ti banked, **0 collected**, identical on 6 maps. 12 purpose-built probe bots. |
...
| G16 | A forward Gunner + one adjacent Harvester kills a 500 HP Core by **turn 77** for ~96 Ti; 13/15 maps have an ore-adjacent firing position. | `ASSERTED` | Single measurement, not independently reproduced. **Probe failed twice to session limits — highest-priority open item.** |
