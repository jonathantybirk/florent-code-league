...
direct probes, because the tutorial mirror in `docs/tutorials/` disagrees with
the engine on three points that matter.

Code (not markdown) lives in `analysis/econ/`; probe bots in `bots/probe_*`.
...
Measured in-engine at L = 3, 6, 10, 13 (`bots/probe_order`, four variants run
...
Both 1.1 and 1.2 trace to the same root cause: trusting `docs/tutorials/`, which
...
**(b) The engine driven by those plans** (`bots/exec_plan`) — one builder reaches
