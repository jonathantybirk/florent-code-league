...
def test_build_rates_each_map_pool_separately_and_counts_core_maps(tmp_path):
    ...
    # Each pool that was played is offered on its own, then the union of the official eras, then
    # that union plus the held-out maps -- not the powerset, which is 31 entries over five pools.
    assert [pool["id"] for pool in index["map_pools"]] == [
        "_legacy", "_official-20260806", "_secret",
        "_legacy_official-20260806",
        "_legacy_official-20260806_secret",
    ]
    # atoll is in the legacy and 6 Aug pools and duel only in legacy, so the official union is two
    # maps, not three: combining pools is a union, and the shared map is not counted twice.
    assert index["field_official-20260806"]["maps"] == 1
    assert index["field_legacy"]["maps"] == 2
    assert index["field_legacy_official-20260806"]["maps"] == 2
    assert index["field_secret"]["maps"] == 1
    assert index["field_legacy_official-20260806_secret"]["maps"] == 3
    ...
    assert index["pool_coverage"]["official-20260806"] == {
        "played": 1, "defined": 15, "label": "Official maps (06 Aug)",
    }
    # An era nobody played is still reported as defined-but-unplayed, so a pool that silently
    # never ran is visible rather than absent -- but it is not offered as a selectable pool.
    assert index["pool_coverage"]["official-20260821"] == {
        "played": 0, "defined": 15, "label": "Official maps (21 Aug, live)",
    }
    assert all("official-20260821" not in pool["id"] for pool in index["map_pools"])

    # Selections covering the same maps share one table instead of shipping copies of it.
    assert index["pool_aliases"]["_legacy"] == ""
    assert index["pool_aliases"]["_legacy_official-20260806"] == ""
    assert "rankings_legacy" not in index
    assert "rankings_legacy_official-20260806" not in index
    ...
    assert core_maps("rankings_official-20260806") == {"alpha": 1, "beta": 0}
    assert core_maps("rankings_secret") == {"alpha": 0, "beta": 1}
    assert core_maps("rankings_legacy_official-20260806_secret") == {"alpha": 2, "beta": 1}
...
def test_build_refuses_to_publish_a_pool_with_unplayed_pairs(tmp_path, capsys):
    ...
    assert "rankings_legacy_official-20260806_secret" in index
