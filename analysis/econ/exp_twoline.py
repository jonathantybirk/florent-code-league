...
def build_map():
    ...
    write_map_with_cores("maps/_twoline.map26", rows, (2, 2), (20, 20))
    return rows
...
def run(tasklist, mapname):
    json.dump({"tasks": tasklist}, open(f"{SCR}/c.json", "w"))
    env = dict(os.environ, FCODE_ECON_CFG=f"{SCR}/c.json")
    out = subprocess.run(["uv", "run", "fcode", "run", "probe_script", "do_nothing_bot",
                          mapname, "--replay", f"{SCR}/x.replay26"],
